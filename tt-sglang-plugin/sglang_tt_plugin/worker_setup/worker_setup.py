...
def get_dp_rank_from_process_title() -> int:
    ...
    title = setproctitle.getproctitle() #extract process title that scheduler gave them in format: "_DP1_TP0"
...
def setup_worker_environment(worker_id: str, cpu_threads: str = "1", num_threads: int = 1):
    ...
    logger.info(f"[TT-Plugin] Worker {worker_id}: TT_METAL_CACHE={worker_cache}, TT_CACHE_PATH={model_cache}")
...
def _setup_galaxy_mesh_config(tt_metal_home: str):
    ...
    if descriptor:
        ...
        logger.info(f"[TT-Plugin] DEVICE_MESH_SHAPE={mesh_shape} -> descriptor={descriptor}")