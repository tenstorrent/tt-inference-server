...
class WhisperService:
    ...
    def _convert_audio_to_wav(self, audio_data: bytes) -> str:
        """Convert audio data to WAV format. Returns path to WAV file."""
        import subprocess
        import shutil
        ...
        if shutil.which('ffmpeg'):
            try:
                ...
                if result.returncode == 0:
                    ...
                    # Get the actual error message from ffmpeg stderr
                    stderr = result.stderr.decode('utf-8', errors='ignore') if result.stderr else 'unknown'
                    # Find the actual error line (usually last few lines)
        ...
        # Try soundfile/scipy (for WAV input)
        try:
            import soundfile as sf
            import numpy as np
            
            # Try reading directly (works if browser sent WAV)
            ...
            sf.write(wav_file.name, data, 16000)
            os.unlink(raw_file.name)
            return wav_file.name
        ...
        # Last resort: try using the raw file directly
    ...
    async def transcribe(self, audio_data: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            wav_path = await loop.run_in_executor(
                None,
                lambda: self._convert_audio_to_wav(audio_data)
            )
