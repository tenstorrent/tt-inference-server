# mobilenetv2 Tenstorrent Support on N150/N300
...
## Quickstart - Deploy mobilenetv2 Inference Server on n150
...
```bash
python3 run.py --model mobilenetv2 --device n150 --workflow server --docker-server
```
...
```bash
python3 run.py --model mobilenetv2 --device n300 --workflow server --docker-server
```
