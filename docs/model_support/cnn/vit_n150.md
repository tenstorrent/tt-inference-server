# vit Tenstorrent Support on N150/N300
...
## Quickstart - Deploy vit Inference Server on n150
...
```bash
python3 run.py --model vit --device n150 --workflow server --docker-server
```
...
```bash
python3 run.py --model vit --device n300 --workflow server --docker-server
```
