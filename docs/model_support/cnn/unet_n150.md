# unet Tenstorrent Support on N150/N300
...
## Quickstart - Deploy unet Inference Server on n150
...
```bash
python3 run.py --model unet --device n150 --workflow server --docker-server
```
...
| Parameter | Value |
|-----------|-------|
| Weights | [unet](https://huggingface.co/unet) |
...
```bash
python3 run.py --model unet --device n300 --workflow server --docker-server
```
