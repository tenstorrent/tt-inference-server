# whisper-large-v3 Tenstorrent Support on BH QuietBox 2
...
To use non-default weights, replace `openai/whisper-large-v3` in commands below.
...
## Quickstart - Deploy whisper-large-v3 Inference Server on BH QuietBox 2
...
```bash
python3 run.py --model openai/whisper-large-v3 --device p300x2 --workflow server --docker-server
```
