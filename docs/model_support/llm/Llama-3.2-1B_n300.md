# Llama-3.2-1B Tenstorrent Support on N300
...
To use non-default weights, replace `meta-llama/Llama-3.2-1B` in commands below.
...
## Quickstart - Deploy Llama-3.2-1B Inference Server on n300
...
```bash
python3 run.py --model meta-llama/Llama-3.2-1B --device n300 --workflow server --docker-server
```
