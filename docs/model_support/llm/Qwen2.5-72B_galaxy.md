...
To use non-default weights, replace `Qwen/Qwen2.5-72B` in commands below.
...
docker run \
  ...
  ghcr.io/tenstorrent/tt-inference-server/vllm-tt-metal-src-release-ubuntu-22.04-amd64:0.3.0-13f44c5-0edd242 \
...
```bash
python3 run.py --model Qwen/Qwen2.5-72B --device galaxy --workflow server --docker-server
```
...
| vLLM Commit | `0edd242` |
| Docker Image | `ghcr.io/tenstorrent/tt-inference-server/vllm-tt-metal-src-release-ubuntu-22.04-amd64:0.3.0-13f44c5-0edd242` |
...
```bash
python3 run.py --model Qwen/Qwen2.5-72B --device galaxy_t3k --workflow server --docker-server
```
