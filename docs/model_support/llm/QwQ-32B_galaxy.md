...
```bash
python3 run.py --model Qwen/QwQ-32B --device galaxy --workflow server --docker-server
```
...
```bash
python3 run.py --model Qwen/QwQ-32B --device galaxy_t3k --workflow server --docker-server
```
