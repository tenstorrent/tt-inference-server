# gemma-3-4b-it Tenstorrent Support on N150
...
To use non-default weights, replace `google/gemma-3-4b-it` in commands below.
...
`gemma-3-4b-it` is also supported on hardware:

- [N300](gemma-3-4b-it_n300.md)
...
```bash
python3 run.py --model google/gemma-3-4b-it --device n150 --workflow server --docker-server
```
