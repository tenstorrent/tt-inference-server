...
To use non-default weights, replace `google/gemma-3-27b-it` in commands below.
...
docker run \
  ...
  ghcr.io/tenstorrent/tt-inference-server/vllm-tt-metal-src-release-ubuntu-22.04-amd64:0.9.0-0b10c51-3499ffa \
...
```bash
python3 run.py --model google/gemma-3-27b-it --device galaxy --workflow server --docker-server
```
...
```bash
python3 run.py --model google/gemma-3-27b-it --device galaxy_t3k --workflow server --docker-server
```
