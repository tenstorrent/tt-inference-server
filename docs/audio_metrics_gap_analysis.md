...
| Voice mix | **Covered** | `voice` label on `tts_requests_total` and `tts_output_audio_seconds_total`: the speaker id the runner reports (falling back to the requested id), `custom` for raw client embeddings, `default` otherwise; truncated to 64 chars. Cardinality = the deployment's speaker catalog: success is what proves a client-named id exists, and error rows are collapsed to `voice="unknown"`, so failed requests can't mint label values from arbitrary client strings. |
...
| Real-time factor | **Covered** | `stt_realtime_factor` per request; time-window RTF is also derivable as `rate(request_duration_sum) / rate(input_audio_seconds_total)`. The denominator is the full submitted duration on both streaming paths (not post-VAD speech-only seconds), so RTF is comparable across the `streaming` label. |
| Word error rate | **Proxy exported; true WER out of scope** | WER needs reference transcripts, so it can't be an HTTP metric — offline evals in `test_module/eval_tests/whisper_eval_test.py` stay the source of truth. Live drift proxies now exported from the whisper runner, once per generation: `stt_avg_logprob`, `stt_no_speech_probability`, `stt_compression_ratio` (the same signals Whisper's temperature-fallback gates on). Caveat: tt-metal only computes the first two on the *un-traced* decode path — on traced decode (the performance path) they come back as dummy zeros and are deliberately skipped, so those two series can be absent on traced deployments; `compression_ratio` (computed server-side from the transcript) is always present. Optional next step: canary WER on known audio via the existing `CanaryMonitor`. |
| Audio duration | **Covered** | `stt_input_audio_duration_seconds` + total. |
| Chunk size | **Covered** | `stt_chunk_audio_seconds{mode}`: one observation per merged chunk at the VAD-merge site — the distribution the pre-existing chunk *count* can't give (it's bimodal: short diarized chunks vs. long VAD-only ones, and chunk length drives effective throughput). Configured merge target exported as `audio_chunk_duration_seconds` on `tt_media_server_info`. VAD-path only: with preprocessing skipped there are no merge-produced chunks (tt-metal frames internally) and the series is legitimately absent. |
| Sample rate / channel count | **Covered** | By request: `stt_requests_by_audio_format_total{sample_rate, channels}` counts successful requests at each submitted operating point (values stamped by preprocessing before the mono/default-rate resample; `unknown` when undeterminable). The pre-existing `audio_feature_extraction_input_seconds_total{sample_rate, channels}` stays, but it answers a different question: it is the audio-seconds numerator of the feature-extraction throughput calculation, and its format labels are *traffic-cohort* dimensions, not performance inputs — the extractor always sees post-resample mono audio at the default rate, so the labels deliberately record what the client submitted. Effective throughput still varies by cohort because chunk cost is fixed per padded 30s frame and different traffic types (e.g. 8kHz telephony vs. 48kHz media) VAD-segment into different chunk lengths. |
