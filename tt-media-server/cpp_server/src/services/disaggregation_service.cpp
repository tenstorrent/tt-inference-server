...
tt::sockets::PrefillRequestMessage buildPrefillRequestMessage(
  ...
  message.fastMode = request.fast_mode;
  ...
  message.decodePositionId = request.kv_position_id.has_value()
                                 ? static_cast<int>(*request.kv_position_id)
                                 : 0;
  // Same reused prefix as decodePositionId but excluding accumulated think
  // tokens folded into kv_position_id during session resolution.
  message.decodeSkipTokens =
      message.decodePositionId - request.accumulated_think_tokens;
  return message;
}
...
void DisaggregationService::handleStreamingRequest(
  ...
  if (mode == tt::config::LLMMode::DECODE_ONLY) {
    ...
    auto message = buildPrefillRequestMessage(request, registrationHashes);
    auto sent = socketService->sendPrefillRequest(
        message.taskId, message.registrationHashes, message.tokenIds,
        message.maxTokens, message.slotId,
        tt::utils::mapper::mapSamplingParams(request), message.decodePositionId,
        message.decodeSkipTokens);
...
void DisaggregationService::handleDynamoStreamingRequest(
    LLMRequest& request, const std::vector<uint64_t>& registrationHashes,
    const StreamCallback& callback) {
  if (mode != tt::config::LLMMode::DECODE_ONLY) {
    throw std::runtime_error(
        "[DisaggregationService] Server must be in decode only mode to handle "
        "Dynamo prefill requests");
  }

  auto message = buildPrefillRequestMessage(request, registrationHashes);
  streamCallbacks.insert(message.taskId, callback);
  ...
  std::thread([this, message = std::move(message), options = std::move(options),
    ...
    try {
      ...
      if (!callback.has_value()) {
        TT_LOG_WARN(
            "[DisaggregationService] Dynamo prefill completed after callback "
            "was removed taskId={}",
            message.taskId);
        return;
      }
      handlePrefillResult(result, callback.value());
    } catch (const std::exception& e) {
      auto callback = streamCallbacks.take(message.taskId);
      if (!callback.has_value()) return;
      TT_LOG_WARN("[DisaggregationService] Dynamo prefill failed taskId={}: {}",
                  message.taskId, e.what());
      callback.value()(makeErrorChunk(message.taskId, "prefill error",
                                      LLMErrorReason::GENERIC),
                       /*isFinal=*/true);
    }
  }).detach();
...
void DisaggregationService::abortRequest(uint32_t taskId) {
  if (mode == tt::config::LLMMode::DECODE_ONLY) {
    ...
    if (!tt::config::dynamoDecodeOrchestratesPrefill()) {
      ...
      if (!sent) {
        TT_LOG_WARN(
            "[DisaggregationService] Failed to send prefill cancel for "
            "task_id: {}",
            taskId);
      }
    }
