...
tt::utils::PrefixCachingInfo computeRoutingInfo(
  ...
  if (auto* tokens = std::get_if<std::vector<int>>(&req.prompt)) {
    ...
    // Static per-model constant — see tokenizers::staticInfoFor(). Never
    // touches a Tokenizer instance, so this hot path doesn't load
    // tokenizer.json on the per-request std::thread that the Dynamo
    // server spawns.
    const auto& header =
