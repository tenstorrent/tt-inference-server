...
std::string env_string_lower(const char* name,
...
ModelService model_service() {
  return model_service_from_string(
      env_string_lower("MODEL_SERVICE", defaults::MODEL_SERVICE));
}
...
bool is_llm_service_enabled() { return model_service() == ModelService::LLM; }

std::string runner_type() { return to_string(model_service()); }

size_t num_workers() { return device_ids_parsed().size(); }

unsigned batch_timeout_ms() {
  return static_cast<unsigned>(
      env_ulong("MAX_BATCH_DELAY_TIME_MS", defaults::MAX_BATCH_DELAY_TIME_MS));
}
...
static std::filesystem::path tokenizers_dir() {
  ...
  if (!ec) {
    std::filesystem::path dir =
        exe_path.parent_path().parent_path() / "tokenizers";
...
std::string tokenizer_path() { return tokenizer_path(model_type()); }
...
LLMConfig llm_engine_config() {
  ...
  std::string backend =
      env_string_lower("LLM_DEVICE_BACKEND", defaults::LLM_DEVICE_BACKEND);
  if (backend == "pipeline") {
...
SchedulingPolicy scheduling_policy() {
  return scheduling_policy_from_string(
      env_string_lower("SCHEDULING_POLICY", defaults::SCHEDULING_POLICY));
}

size_t max_in_flight_count() {
  return static_cast<size_t>(
      env_ulong("MAX_IN_FLIGHT_COUNT", defaults::MAX_IN_FLIGHT_COUNT));
}
...
bool enable_accumulated_streaming() {
  return env_ulong("ENABLE_ACCUMULATED_STREAMING",
                   defaults::ENABLE_ACCUMULATED_STREAMING);
}

size_t max_accumulated_tokens() {
  return static_cast<size_t>(
      env_ulong("MAX_ACCUMULATED_TOKENS", defaults::MAX_ACCUMULATED_TOKENS));
}
...
size_t max_queue_size() {
  return static_cast<size_t>(
      env_ulong("MAX_QUEUE_SIZE", defaults::MAX_QUEUE_SIZE));
}
