...
BlazeDecodeRunner::BlazeDecodeRunner(
  ...
  if (config.runner_type == tt::config::ModelRunnerType::MOCK_PIPELINE &&
    ...
    TT_LOG_INFO(
        "BlazeDecodeRunner: using MockDecodeScheduler (single-threaded)");
    decodeScheduler = std::make_unique<MockDecodeScheduler>(maxUsers);
  } else {
    TT_LOG_INFO(
...
void BlazeDecodeRunner::run() {
  while (!stopped.load(std::memory_order_relaxed)) {
    try {
      ...
      TT_LOG_ERROR("BlazeDecodeRunner: Exception in run: {}", e.what());
      throw;
    }
  }
}

bool BlazeDecodeRunner::warmup() {
  ...
  while (!decodeScheduler->try_pop_response(response)) {
    if (std::chrono::steady_clock::now() >= allocateDeadline) {
      TT_LOG_ERROR(
          "[BlazeDecodeRunner] Warmup timed out waiting for ALLOCATE response "
          "after "
          "{} ms",
          timeout.count());
      return false;
  ...
  TT_LOG_INFO("BlazeDecodeRunner: warmup - got slot_id={}", slotId);
  if (slotId == ds::INVALID_SLOT) {
    TT_LOG_ERROR("BlazeDecodeRunner: Warmup failed with error");
    return false;
  }
  ...
  if (!receivedToken) {
    TT_LOG_ERROR(
        "[BlazeDecodeRunner] Warmup timed out waiting for token after {} ms",
        timeout.count());
    return false;
  }

  TT_LOG_INFO(
      "BlazeDecodeRunner: warmup - pushing EVICT request (slotId={})...",
      slotId);
  decodeScheduler->push_request(
  ...
  while (!decodeScheduler->try_pop_response(evictResponse)) {
    if (std::chrono::steady_clock::now() >= evictDeadline) {
      TT_LOG_ERROR(
          "[BlazeDecodeRunner] Warmup timed out waiting for EVICT ack after {} "
          "ms "
          "(slotId={})",
  ...
  TT_LOG_INFO("BlazeDecodeRunner: warmup - got EVICT ack (slotId={})", slotId);
  TT_LOG_INFO("BlazeDecodeRunner: Warmup successful");
  return true;
}

void BlazeDecodeRunner::stop() {
...
void BlazeDecodeRunner::step() {
  ...
  if (memoryRequest.has_value()) {
    TT_LOG_DEBUG(
        "[BlazeDecodeRunner] step: got memoryRequest taskId={}, action={}",
...
inline void BlazeDecodeRunner::handleMemoryRequest(
  ...
  switch (request.action) {
    ...
    default: {
      TT_LOG_ERROR(
          "[BlazeDecodeRunner] handleMemoryRequest: unexpected action for "
          "taskId={}, "
          "action={}",
...
inline void BlazeDecodeRunner::handleEvictRequest(
    const tt::domain::ManageMemoryTask& request) {
  ...
  switch (slotContext.state) {
    ...
    case SlotState::RUNNING: {
      if (!decodeScheduler->push_request(evictRequest)) {
        TT_LOG_WARN(
            "[BlazeDecodeRunner] handleEvictRequest: scheduler queue full, "
            "deferring "
      ...
      TT_LOG_DEBUG(
          "[BlazeDecodeRunner] handleEvictRequest: pushed EVICT taskId={}, "
          "slotId={} "
          "(was {})",
    ...
    case SlotState::AWAITING_EVICT_ACK:
      TT_LOG_WARN(
          "[BlazeDecodeRunner] handleEvictRequest: duplicate DEALLOCATE for "
          "slotId={}"
    ...
    case SlotState::FREE:
      TT_LOG_ERROR(
          "[BlazeDecodeRunner] handleEvictRequest: DEALLOCATE for FREE "
          "slotId={}, "
          "taskId={}",
...
inline void BlazeDecodeRunner::handleAllocateAck(uint32_t taskId,
  ...
  TT_LOG_DEBUG("[BlazeDecodeRunner] handleAllocateAck: taskId={}, slotId={}",
               taskId, slotId);
...
inline SlotContext* BlazeDecodeRunner::validateAck(uint32_t taskId,
  ...
  if (slot.pendingAckRequestId != taskId) {
    ...
    TT_LOG_ERROR(
        "[BlazeDecodeRunner] {}: stale ack taskId={} for slotId={} (state={}, "
...
inline void BlazeDecodeRunner::handleEvictAck(uint32_t taskId,
                                              uint32_t slotId) {
  auto* slot = validateAck(taskId, slotId, "handleEvictAck");
  if (!slot) return;
  TT_LOG_DEBUG(
      "[BlazeDecodeRunner] handleEvictAck: taskId={}, slotId={}; clearing slot",
      taskId, slotId);
  slotManager.clearSlotContext(slot->slotId);
}

inline void BlazeDecodeRunner::handleStopAck(uint32_t taskId, uint32_t slotId) {
...
BlazeDecodeRunner::getMemoryRequest() {
...
void BlazeDecodeRunner::drainAndHandleStopRequests() {
  if (pendingRequests.pendingCancelTaskId.has_value()) {
    ...
    TT_LOG_DEBUG(
        "[BlazeDecodeRunner] drainAndHandleStopRequests: retrying deferred "
        "cancel "
        "for taskId={}",
        retryTaskId);
...
inline void BlazeDecodeRunner::handleStopRequest(uint32_t taskId) {
  if (pendingRequests.pendingTask &&
      pendingRequests.pendingTask->taskId == taskId) {
    TT_LOG_DEBUG(
        "[BlazeDecodeRunner] handleStopRequest: dropping queued retry for "
        "taskId={}",
        taskId);
    pendingRequests.pendingTask.reset();
    return;
  ...
  if (!slot) {
    ...
    TT_LOG_DEBUG(
        "[BlazeDecodeRunner] handleCancelRequest: taskId={} has no bound slot "
  ...
  if (slot->state != SlotState::RUNNING) {
    ...
    TT_LOG_DEBUG(
        "[BlazeDecodeRunner] handleCancelRequest: taskId={}, slotId={} not "
        "RUNNING "
        "(state={}); ignoring",
...
void BlazeDecodeRunner::handleSchedulerOutput(const ds::OutputMessage& output) {
  ...
  if (slotContext.state != SlotState::RUNNING) {
    if (slotContext.state == SlotState::AWAITING_STOP_ACK ||
      ...
      TT_LOG_DEBUG(
          "[BlazeDecodeRunner] handleOutput: dropping token for slotId={} "
          "(state={}, "
    ...
    TT_LOG_ERROR(
        "[BlazeDecodeRunner] handleOutput: unexpected token for slotId={} in "
...
void BlazeDecodeRunner::handleTask(
  ...
  switch (slotContext.state) {
    ...
    case SlotState::AWAITING_STOP_ACK: {
      if (slotContext.deferredContinue) {
        TT_LOG_WARN(
            "[BlazeDecodeRunner] handleRequest: overwriting deferred taskId={} "
            "with "
