...
bool EmbeddingRunner::warmup() {
  TT_LOG_INFO(
      ...
      deviceId_, visibleDevice_);

  if (!impl->initPython()) {
    return false;
  }

  if (!impl->importModules()) {
    return false;
  }

  if (!impl->createRunnerInstance()) {
    return false;
  }

  // Initialize the TTNN device before warmup
  if (!impl->callSetDevice()) {
    return false;
  }
  ...
  TT_LOG_INFO("[EmbeddingRunner] Warmup complete for device {}", deviceId_);
  return true;
}
...
std::vector<domain::EmbeddingResponse> EmbeddingRunner::run(
    const std::vector<domain::EmbeddingRequest>& requests) {
  if (!impl || !impl->runner_instance) {
    TT_LOG_ERROR("[EmbeddingRunner] Runner not initialized");
    return {};
  }

  return impl->runInference(requests);
}
