...
bool QwenModelRunner::initialize() {
  if (!Py_IsInitialized()) {
  ...
  try {
    ...
    py::module_ hfMod = py::module_::import("tt_model_runners.hf_cpu_runner");
    gStepSeqClass = hfMod.attr("StepSequence");
    py::object runnerClass = hfMod.attr("HFCPURunner");
    ...
    if (!warmupOk) {
      TT_LOG_ERROR("[QwenModelRunner] Warmup failed");
    } else {
      TT_LOG_INFO("[QwenModelRunner] HF CPU runner ready (in-process)");
      initialized = true;
      success = true;
    }
  } catch (const py::error_already_set& e) {
    TT_LOG_ERROR("[QwenModelRunner] Python init error: {}", e.what());
  }
...
void QwenModelRunner::failSequences(const std::vector<Sequence*>& seqs) {
...
QwenModelRunner::QwenModelRunner(const Config& config, DecodeCallback callback)
    : config(config), decodeCallback(std::move(callback)) {
  initialize();
}

QwenModelRunner::~QwenModelRunner() { exit(); }

void QwenModelRunner::run(const std::vector<Sequence*>& seqs, bool isPrefill) {
  ...
  bool hadError = false;
  {
    ...
    try {
      ...
      for (size_t i = 0; i < seqs.size(); ++i) {
        ...
        if (drIsError) {
          TT_LOG_ERROR("[QwenModelRunner] sequence {} error: {}", drTaskId,
                       error);
        }
      ...
      TT_LOG_ERROR("[QwenModelRunner] Python error in run_step: {}", e.what());
      hadError = true;
    }
  }
  if (hadError) {
...
void QwenModelRunner::exit() {
  ...
  initialized = false;
  TT_LOG_INFO("[QwenModelRunner] Runner exited");
}

std::unique_ptr<IModelRunner> makeQwenModelRunner(const Config& config,
                                                  DecodeCallback callback) {
  auto runner = std::make_unique<QwenModelRunner>(config, std::move(callback));
  if (!runner->isReady()) {
    return nullptr;
  }
  return runner;
