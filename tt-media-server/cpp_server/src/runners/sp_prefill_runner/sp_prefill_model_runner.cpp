...
std::optional<llm_engine::TokenResult> SpPrefillModelRunner::forward(
  ...
  TT_LOG_DEBUG(
      "SpPrefillModelRunner: Writing into shared memory input task_id={}, token count={}",
      taskId, tokenIds.size());
  deviceInput.write(taskId, tokenIds, 1);

  tt::ipc::ReadResult readBuf;
  TT_LOG_DEBUG("SpPrefillModelRunner: Reading from shared memory output task_id={}", taskId);
  while (!stop.load(std::memory_order_relaxed)) {
    if (deviceOutput.tryRead(readBuf)) {
      uint64_t tokenId = readBuf.tokenIds.empty() ? 0 : readBuf.tokenIds[0];
      TT_LOG_DEBUG("SpPrefillModelRunner: Read from shared memory output task_id={}, token_id={}, token count={}", taskId, tokenId, readBuf.tokenIds.size());
