...
Sequence::Sequence(TaskID taskId, int blockSize,
    ...
    : task_id(std::move(taskId)),
      ...
      numPromptTokens_(token_ids_.size()),
...
std::vector<int64_t> Sequence::block(size_t i) const {
  ...
  size_t end = std::min(start + blockSize, token_ids_.size());
  return std::vector<int64_t>(token_ids_.begin() + start,
                              token_ids_.begin() + end);
}

std::vector<int64_t> Sequence::completionTokenIds() const {
  if (numPromptTokens_ >= token_ids_.size()) {
    return {};
  }
  return std::vector<int64_t>(token_ids_.begin() + numPromptTokens_,
                              token_ids_.end());
}
