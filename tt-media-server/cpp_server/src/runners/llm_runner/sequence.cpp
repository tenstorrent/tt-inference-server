...
Sequence::Sequence(TaskID taskId, int blockSize, std::vector<int64_t> tokenIds,
    ...
    : taskId(std::move(taskId)),
      ...
      blockSize(blockSize) {
  assert(this->taskId.id != 0 && "Sequence requires a non-zero task_id");
...
void Sequence::serialize(std::ostream& os) const {
  ...
  os.write(reinterpret_cast<const char*>(&taskId.id), sizeof(taskId.id));
...
Sequence* Sequence::deserialize(std::istream& is) {
  uint32_t taskIdVal = 0;
  is.read(reinterpret_cast<char*>(&taskIdVal), sizeof(taskIdVal));

  Config defaultConfig;
  Sequence* seq =
      new Sequence(TaskID(taskIdVal),
