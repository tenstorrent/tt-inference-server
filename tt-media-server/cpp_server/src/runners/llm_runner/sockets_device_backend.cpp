...
static std::vector<char> token_to_h2d_page(const TaskID& task_id, int64_t token_id) {
  std::vector<char> page(kSocketPageSizeInBytes, 0);
  auto id_bytes = task_id.serialize();
  std::copy(id_bytes.begin(), id_bytes.end(), page.begin());
  std::memcpy(page.data() + TaskID::kSerializedSize, &token_id, sizeof(token_id));
  return page;
}
...
class SocketsDeviceBackend : public IDeviceBackend {
 public:
  ...
  void write(const Sequence& seq) override {
    ...
    auto page = token_to_h2d_page(seq.task_id, token_id);
