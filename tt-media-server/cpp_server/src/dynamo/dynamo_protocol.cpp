...
void DynamoServer::process_request(int fd, const TcpRequestMessage& msg) {
  ...
  //
  // NOTE: we tried a fixed-size handler pool (condvar + notify_one) to
  // amortize thread creation, but it backfired on sparse arrival
  // patterns: each notify_one wakes a worker whose CPU has had time to
  // sink into a deep C-state, paying 5–15 ms of wakeup + context-switch
  // latency that dwarfs the ~50µs we save on `clone()`. A fresh
  // detached thread starts on whatever CPU just spawned it — typically
  // the still-hot accept thread — so wakeup latency is zero. Until we
  // have a real high-RPS workload where clone() shows up in a profile,
  // the per-request thread is the better trade.
