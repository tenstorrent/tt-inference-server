...
std::vector<uint8_t> serializeEngineHandoff(const std::vector<uint8_t>& tableBlob,
                                            const DeviceMap& deviceMap);

/// Parse a handoff blob. nullopt on truncation / malformed bytes. Does NOT
/// deserialize the table (callers that want an IKvTable use receiveEngineHandoff
/// or deserializeKvTable), so this stays usable without ENABLE_KV_TABLE.
