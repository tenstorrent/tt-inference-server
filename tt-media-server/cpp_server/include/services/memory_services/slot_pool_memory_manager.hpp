...
class SlotPoolMemoryManager : public MemoryManager {
 public:
  explicit SlotPoolMemoryManager(uint32_t poolSize);

  void handleRequest(const domain::ManageMemoryTask& request) override;

  uint32_t getPoolSize() const { return slotPoolSize; }
  uint32_t getFreeCount() const { return static_cast<uint32_t>(freeSlots.size()); }
