...
// raw bytes returned by tt::test::dynamo::sendDynamoRequest so tests can
// assert on status code, headers, JSON body, or SSE event stream — instead
// of hand-rolled string searches.
//
//   auto response = HttpResponse::parse(sendDynamoRequest(...));
