...
TEST_F(DeepseekTokenizerTest, ApplyChatTemplateReasoningDisabledInjectsThinkBlock) {
  std::vector<ChatMessage> messages = {
      {"user", "Hello"},
  };
  ...
  std::string actual = tokenizer().applyChatTemplate(
      messages, true, std::nullopt, false);
...
TEST_F(DeepseekTokenizerTest, ApplyChatTemplateReasoningEnabledNoThinkBlock) {
  ...
  std::string actual = tokenizer().applyChatTemplate(
      messages, true, std::nullopt, true);
