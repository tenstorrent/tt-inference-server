...
static const char* kQueueName = "tt_ipc_scheduler_smoke_test";
...
int main() {
  ...
  ipc::message_queue::remove(kQueueName);

  // --- Create the IPC queue (simulates the main server). ---
  ipc::message_queue rawQueue(ipc::create_only, kQueueName, MAX_NUM_MSGS,
                              MAX_MSG_SIZE);
  ...
  {
    tt::ipc::BoostIpcTaskQueue producer(kQueueName);
    producer.push(seq1);
    producer.push(seq2);
  }
  ...
  if (pid == 0) {
    ...
    auto queue = std::make_unique<tt::ipc::BoostIpcTaskQueue>(kQueueName);
