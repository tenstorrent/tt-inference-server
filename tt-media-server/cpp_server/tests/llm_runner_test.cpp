...
TEST(LLMRunnerTest, AllTokensPublishedInOrder) {
  ...
  for (const auto& req : requests) {
    Sequence& seq = engine.getScheduler().addRequest(
  ...
  const std::vector<int64_t> EXPECTED_SEQ_0 = {
      223, 224, 225, 226, 227, 228, 229, 230, 231, 232, 233, 234, 235, 236, 237,
      238, 239, 240, 241, 242, 243, 244, 245, 246, 247, 248, 249, 250, 251, 252,
  };
  const std::vector<int64_t> EXPECTED_SEQ_1 = {
      223, 224, 225, 226, 227, 228, 229, 230, 231, 232,
  };
  const std::vector<int64_t> EXPECTED_SEQ_2 = {
      223, 224, 225, 226, 227, 228, 229, 230, 231, 232,
      233, 234, 235, 236, 237, 238, 239, 240, 241, 242,
  };

  EXPECT_EQ(receivedTokens[taskIds[0]], EXPECTED_SEQ_0);
  EXPECT_EQ(receivedTokens[taskIds[1]], EXPECTED_SEQ_1);
  EXPECT_EQ(receivedTokens[taskIds[2]], EXPECTED_SEQ_2);
