...
# bringup_mooncake_worker. The topology is configurable via env:
#
#   NUM_PREFILL prefill workers + NUM_DECODE decode workers (defaults 4 + 16).
#   Total ranks = NUM_PREFILL + NUM_DECODE — must match `mpirun -np`.
#
#   ranks [0, NUM_PREFILL)      -> prefill-0 .. prefill-(NUM_PREFILL-1)
#   ranks [NUM_PREFILL, total)  -> decode-0  .. decode-(NUM_DECODE-1)
#
#   decode-d  peer  = prefill-(d % NUM_PREFILL)        (round-robin, exactly one)
#   prefill-p peers = every decode-d with d % NUM_PREFILL == p
#
# Round-robin keeps the fan-out balanced and, as long as NUM_DECODE >=
# NUM_PREFILL, guarantees every worker has >=1 peer — so `total` "CONNECTED"
# lines proves the whole mesh wired up. (If NUM_PREFILL > NUM_DECODE the surplus
# prefill workers get no peers and reach READY immediately.) Required env:
# WORKER_BIN, METADATA, HOST_DRAM_BYTES, DISCOVERY_TIMEOUT_SEC.
set -euo pipefail
...
if (( rank < NUM_PREFILL )); then
  ...
  for (( decode_index = 0; decode_index < NUM_DECODE; decode_index++ )); do
    if (( decode_index % NUM_PREFILL == prefill_index )); then
      peer_args+=(--peer "decode-${decode_index}")
    fi
  done
else
  decode_index=$(( rank - NUM_PREFILL ))
  prefill_index=$(( decode_index % NUM_PREFILL ))
