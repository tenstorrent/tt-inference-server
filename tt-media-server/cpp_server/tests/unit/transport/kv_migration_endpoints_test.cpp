...
constexpr auto kRecvTimeout = 1000ms;
...
TEST(KvControlChannelConnectorTest, OpensOneChannelPerEndpoint) {
  std::unordered_map<std::string, Endpoint> eps{
      {"D0", {"10.0.0.1", 7001}}, {"D1", {"10.0.0.2", 7002}}};

  std::unordered_map<uint16_t, Endpoint> seen;
  KvControlChannelConnector connector(
      eps, [&](const Endpoint& ep) -> std::shared_ptr<sockets::ISocketTransport> {
        seen[ep.port] = ep;
...
TEST(KvControlChannelConnectorTest, SkipsHostWhenFactoryFailsButKeepsRest) {
  ...
  EXPECT_FALSE(connector.connect());  // not every endpoint connected
  auto channels = connector.channels();
...
TEST(KvMigrationEndpoints, SingleHostMigratesThroughConnectorAndServer) {
  ...
  KvMigrationMultiHostSender multiHost(senderEngine, prefillDev, prefill, decode,
                                       "P", connector.channels());
...
TEST(KvMigrationEndpoints, TwoHostFanOutThroughConnectorAndServers) {
  ...
  KvControlChannelConnector connector(
      ...
      kRecvTimeout, kPollInterval);
  ASSERT_TRUE(connector.connect());
  EXPECT_EQ(connector.connectedCount(), 2u);
