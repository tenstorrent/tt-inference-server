...
// The keystone of Phase 5a: a Kafka-shaped tt::services::MigrationRequest,
// handed to the real Mooncake-backed IMigrationExecutor, drives the whole
// multi-host data plane and byte-lands on both decode hosts — proving the
// executor is no longer a stub. Same fan-out as FansOutToTwoDecodeHosts, but
...
TEST(KvMigrationMultiHost, DrivenThroughMooncakeExecutor) {
  ...
  exec.execute(0x5151, api, [&](tt::services::MigrationStatus s) {
    done.set_value(s);
  });
