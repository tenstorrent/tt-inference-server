...
TEST(DynamoProtocolTest, DataChunkCarriesDisaggregatedParams) {
  tt::dynamo::TokenChunk chunk;
  chunk.finish_reason = "stop";
  chunk.disaggregated_params["kv_transfer_params"]["tt_prefill_handoff"]
                            ["selected_prefill_id"] = "prefill/generate/abc";
  chunk.disaggregated_params["kv_transfer_params"]["tt_prefill_handoff"]
                            ["migration_id"] = Json::UInt64(1234);
  ...
  const Json::Value& handoff =
      output["disaggregated_params"]["kv_transfer_params"]
            ["tt_prefill_handoff"];
