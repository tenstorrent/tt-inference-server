...
class TelemetryClient:
    ...
    def record_telemetry_event(self, event_name: TelemetryEvent, device_id: str = None, duration: float = None, status: bool = True):
        """Record a telemetry event"""

        status_str = "success" if status else "failure"

        if event_name == TelemetryEvent.PRE_PROCESSING:
            self._record_pre_processing(duration, preprocessing_enabled=True, status=status_str)
        elif event_name == TelemetryEvent.POST_PROCESSING:
            self._record_post_processing(duration, preprocessing_enabled=True, status=status_str)
            ...
            self._record_base_total_request_duration(device_id, duration, status=status_str)
        elif event_name == TelemetryEvent.BASE_SINGLE_PROCESSING:
            self._record_single_base_request_duration(device_id, duration, status=status_str)
        else:
    ...
    def _record_base_total_request_duration(self, device_id: str, duration: float, status: str = "success"):
        """Record end to end duration"""
        total_base_request_duration.labels(
            model_type=self.settings.model_runner
        ).observe(duration)
        
        total_base_request_counter.labels(
            model_type=self.settings.model_runner
        ).inc()

    def _record_single_base_request_duration(self, device_id: str, duration: float, status: str = "success"):
        """Record end to end duration"""
        base_request_duration.labels(
            model_type=self.settings.model_runner
        ).observe(duration)
        
        base_request_counter.labels(
            model_type=self.settings.model_runner
        ).inc()
