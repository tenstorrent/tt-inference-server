...
# Ordered: the first entry is the canonical model for the runner, used when MODEL
# is unset. Tuples rather than sets — Enum members hash by identity, so iterating a
# set here made the resolved weights path vary with PYTHONHASHSEED (a SP_RUNNER
# deployment could come up on T2V or I2V weights depending on the run).
