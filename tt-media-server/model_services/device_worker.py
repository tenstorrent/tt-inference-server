...
def device_worker(
    ...
    # Create a persistent event loop for the entire worker process lifecycle
    ...
    try:
        ...
        # Use the persistent loop instead of asyncio.run() to keep AsyncLLMEngine alive
        try:
    ...
    while True:
        ...
        if inference_requests[0] is None:  # Sentinel to shut down
            ...
            # Clean up the event loop before shutdown
            loop.close()
            break
        logger.info(
