...
class Scheduler:
    ...
    def process_request(self, request):
        try:
            ...
            try:
                self.logger.info(f"Putting request {request._task_id} into task queue")
    ...
    async def result_listener(self):
        while self.listener_running:
            try:
                ...
                self.logger.info(
                    f"Result received from result queue: {result_key} from worker {worker_id}"
                )
                ...
                self.logger.info(f"Result queue found for task {task_id}: {queue}")

                if queue:
                    self.logger.info(f"Putting result for task {task_id} into queue")
