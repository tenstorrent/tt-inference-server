...
from ...tools.utils import print_compiled_model_results, get_label_for_index, yolo_postprocess
...
class ModelLoader(ForgeModel):
    ...
    def output_to_prediction(self, output):
        ...
        # lazy load COCO YAML
        if not self.coco_yaml:
            yaml_url = (
                "https://raw.githubusercontent.com/ultralytics/yolov5/master/data/coco.yaml"
            )
            response = requests.get(yaml_url)
            self.coco_yaml = yaml.safe_load(response.text)

        class_names = self.coco_yaml["names"]
        det = processed_output[0]

        ## Debug: print all detections, we wil just use the top detection for now
        # print("Detections:")
        # for d in det:
        ...
        predicted_label = class_names[int(det[0][5])] if len(det) > 0 else "No detections"
        compiled_model_top1_class_prob = (det[0][4] if len(det) > 0 else 0.0) * 100
        return [{
