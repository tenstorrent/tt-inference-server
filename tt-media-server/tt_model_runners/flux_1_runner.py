...
import ttnn
from models.experimental.tt_dit.pipelines.flux1.pipeline_flux1 import create_pipeline
from domain.image_generate_request import ImageGenerateRequest

class TTFlux1Runner(BaseDeviceRunner):
    ...
    @log_execution_time("Flux1 warmup")
    async def load_model(self, device)->bool:
        self.logger.info(f"Device {self.device_id}: Loading model...")

        if self.mesh_device != device:
            raise Exception(f"Device {self.device_id}: Passed in device is not the same as device used for Flux1 runner initialization")
    ...
    @log_execution_time("Flux1 inference")
