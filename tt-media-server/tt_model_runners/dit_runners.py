...
def _ltx23_pipeline_args(request, resolution, output_path: str) -> dict:
    # Distilled generate() has no steps/negative_prompt knob (fixed sigma schedule).
    pipeline_args = {
        "prompt": request.prompt,
...
class TTLtx23DistilledRunner(TTDiTRunner):
    def __init__(self, device_id: str):
        super().__init__(device_id)
        self.resolution = ltx23_target_resolution(self.settings.device_mesh_shape)

    def create_pipeline(self):
        try:
            # Traced replay is broken on ltx-perf (kwargs mismatch in tracing.py); default off.
            traced = os.environ.get("LTX_TRACED", "0") in ("1", "true", "True")
            return LTXDistilledPipeline.create_pipeline(
                mesh_device=self.ttnn_device,
                checkpoint_name=f"{self.settings.model_weights_path}:{LTX23_DISTILLED_FILENAME}",
    ...
    def run(self, requests: list[VideoGenerateRequest]):
        ...
        result_path = self.pipeline.generate(
            **_ltx23_pipeline_args(requests[0], self.resolution, output_path)
        )
        self.logger.debug(f"Device {self.device_id}: Inference completed")
        # AV mp4 written in-pipeline; return [path] (VideoService passes str through, keeping audio).
        return [result_path]
