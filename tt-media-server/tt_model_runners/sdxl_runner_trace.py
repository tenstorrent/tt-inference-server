...
class TTSDXLRunnerTrace(BaseDeviceRunner):
    ...
    def run_inference(self, requests: list[ImageToImageRequest]):
        ...
        base64_data = requests[0].image
        torch_image = ImageSearchRequest(prompt=base64_data).get_pil_image()

        images = self.tt_sdxl.torch_pipeline.image_processor.preprocess(
            torch_image, height=1024, width=1024, crops_coords=None, resize_mode="default"
        )
        ...
        transform = transforms.Compose([
            transforms.ToTensor(),  # Converts to (3, 1024, 1024) and scales to [0,1]
        ])
