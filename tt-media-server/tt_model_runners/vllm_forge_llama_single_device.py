...
class VLLMForgeLlamaSingleDeviceRunner(BaseDeviceRunner):
    """Optimized vLLM runner for single-device Llama models (1B, 3B, 8B).

    Hardcodes compiler optimizations that have been validated for Llama on
    single TT devices: consteval, bfp8 weights, CPU sampling, and
    optimization level 1 (enables RoPE and other fusions).
    """
    ...
    @log_execution_time(
        "VLLM Llama model load",
        TelemetryEvent.DEVICE_WARMUP,
        os.environ.get("TT_VISIBLE_DEVICES"),
    )
    async def warmup(self) -> bool:
        self.logger.info(f"Device {self.device_id}: Loading VLLM Llama model...")
    ...
    async def _run_async(self, requests: list[CompletionRequest]):
        try:
            self.logger.debug(f"Device {self.device_id}: Running Llama inference")
            request = requests[0]
            if request.stream:
            ...
            self.logger.error(
                f"Device {self.device_id}: Llama inference failed: {type(e).__name__}: {e}"
            )
            self.logger.error(
                f"Device {self.device_id}: Full traceback: {traceback.format_exc()}"
            )
            raise RuntimeError(f"Llama inference failed: {str(e)}") from e
    ...
    async def process_request_non_streaming(
        ...
        self.logger.info(
            f"Device {self.device_id}: Starting non-streaming generation"
        )
