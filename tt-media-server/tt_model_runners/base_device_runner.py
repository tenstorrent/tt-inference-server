...
class BaseDeviceRunner(ABC):
    def __init__(
        ...
        self.logger.info(
            f"BaseDeviceRunner.__init__: device_id={device_id!r}, "
            f"model_runner={self.settings.model_runner!r}, "
            f"mesh_shape={self.settings.device_mesh_shape}"
        )
        ...
        if self.device_id != "-1":
            ...
            self.logger.info(
                f"BaseDeviceRunner: setting up runner env "
                f"(cpu_threads={cpu_threads}, torch_threads={num_torch_threads})"
            )
        ...
        self.logger.info(
            f"BaseDeviceRunner.__init__ complete: device_id={device_id}, "
