...
class VLLMForgeLlama70BRunner(BaseDeviceRunner):
    def __init__(self, device_id: str):
        super().__init__(device_id)

    @log_execution_time(
        "VLLM Forge Llama 70B model load",
        TelemetryEvent.DEVICE_WARMUP,
        os.environ.get("TT_VISIBLE_DEVICES"),
    )
    async def warmup(self) -> bool:
        self.logger.info(
            f"Device {self.device_id}: Loading VLLM Forge Llama 70B model..."
        )
    ...
    @log_execution_time(
        "Run VLLM Forge Llama 70B inference",
    ...
    async def _run_async(self, requests: list[CompletionRequest]):
        try:
            self.logger.debug(f"Device {self.device_id}: Running Llama 70B inference")
            ...
            self.logger.error(
                f"Device {self.device_id}: Llama 70B inference failed: {type(e).__name__}: {e}"
            )
            self.logger.error(
                f"Device {self.device_id}: Full traceback: {traceback.format_exc()}"
            )
            raise RuntimeError(f"Llama 70B inference failed: {str(e)}") from e
    ...
    async def _generate_streaming(self, request: CompletionRequest):
        ...
        self.logger.info(
            f"Device {self.device_id}: Llama 70B streaming generation completed"
        )
    ...
    async def process_request_non_streaming(
        ...
        self.logger.info(
            f"Device {self.device_id}: Starting Llama 70B non-streaming generation"
        )
        ...
        self.logger.info(
            f"Device {self.device_id}: Llama 70B non-streaming generation completed"
        )
