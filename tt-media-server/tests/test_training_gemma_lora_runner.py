...
class TestTrainingGemmaLoraRunner:
    ...
    def runner(self):
        ...
        with patch("tt_model_runners.base_device_runner.get_settings"), patch(
            "tt_model_runners.base_device_runner.TTLogger"
        ):
            return TrainingGemmaLoraRunner(device_id="test_device_0")

    @patch(
        "tt_model_runners.forge_training_runners.training_gemma_lora_runner.AutoModelForCausalLM"
    )
    @patch(
        "tt_model_runners.forge_training_runners.training_gemma_lora_runner.get_dataset_loader"
    )
    @patch("tt_model_runners.forge_training_runners.training_gemma_lora_runner.xr")
    def test_warmup_configures_hardware_and_loading(
        self, mock_xr, mock_get_dataset, mock_hf, runner
    ):
        ...
        mock_get_dataset.assert_any_call(
            runner.model_name, split="train", collate_fn=pytest.any
        )

    @patch(
        "tt_model_runners.forge_training_runners.training_gemma_lora_runner.get_peft_model"
    )
    @patch(
        "tt_model_runners.forge_training_runners.training_gemma_lora_runner.torch_xla.sync"
    )
    def test_run_training_loop_execution(
        self, mock_sync, mock_get_peft, runner, mock_training_request
    ):
