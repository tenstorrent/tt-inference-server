...
class TestScheduler:
    ...
    def scheduler(self, mock_queues, mock_locks):
        ...
        with patch('multiprocessing.Queue') as mock_queue_constructor, \
             patch('threading.Lock') as mock_lock_constructor:
            ...
            mock_lock_constructor.side_effect = lambda: lock_sequence.pop(0) if lock_sequence else create_mock_lock()
    ...
    def test_process_request_queue_put_timeout(self, scheduler):
        ...
        with patch.object(scheduler.task_queue, 'put', side_effect=Exception("Timeout")):
            # Execute and verify
            with pytest.raises(Exception) as exc_info:
                scheduler.process_request(mock_request)
    ...
    async def test_error_listener(self, mock_to_thread, scheduler, mock_future):
        ...
        mock_to_thread.side_effect = [
            ...
            (test_worker_id, None, None)  # worker_id must be valid for error_count increment
        ]
    ...
    @patch('asyncio.create_task')
    @patch('asyncio.to_thread')
    async def test_device_warmup_listener(self, mock_to_thread, mock_create_task, scheduler):
    ...
    def test_stop_workers(self, scheduler):
        ...
        mock_process1.is_alive = Mock(return_value=True)  # Worker is alive, should be joined
        
        mock_process2 = Mock(spec=Process)
        mock_process2.join = Mock()
        mock_process2.is_alive = Mock(return_value=True)  # Worker is alive, should be joined
        ...
        # Patch queue methods
        with patch.object(scheduler.task_queue, 'put') as mock_task_put, \
             patch.object(scheduler.task_queue, 'close') as mock_task_close, \
             patch.object(scheduler.task_queue, 'join_thread') as mock_task_join, \
             patch.object(scheduler.result_queue, 'put') as mock_result_put, \
             patch.object(scheduler.result_queue, 'close') as mock_result_close, \
             patch.object(scheduler.result_queue, 'join_thread') as mock_result_join, \
             patch.object(scheduler.warmup_signals_queue, 'put') as mock_warmup_put, \
             patch.object(scheduler.warmup_signals_queue, 'close') as mock_warmup_close, \
             patch.object(scheduler.warmup_signals_queue, 'join_thread') as mock_warmup_join, \
             patch.object(scheduler.error_queue, 'put') as mock_error_put, \
             patch.object(scheduler.error_queue, 'close') as mock_error_close, \
             patch.object(scheduler.error_queue, 'join_thread') as mock_error_join:
