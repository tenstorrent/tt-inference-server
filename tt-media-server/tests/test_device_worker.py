...
class TestDeviceWorker:
    ...
    def test_device_worker_initialization_failure(self, mock_queues):
        ...
        error_queue.put.assert_called_once_with(("worker_0", -1, "Device initialization failed"))
    ...
    def test_device_worker_successful_inference(self, mock_timer, mock_get_batch, mock_queues, mock_requests):
        ...
        fresh_device_runner.run_inference.return_value = [Mock(), Mock()]  # Return mock images
        
        with patch('model_services.device_worker.get_device_runner', return_value=fresh_device_runner):
            with patch('model_services.device_worker.get_telemetry_client', Mock()):
    ...
    def test_device_worker_inference_error(self, mock_timer, mock_get_batch, mock_queues, mock_requests):
        ...
        with patch('model_services.device_worker.get_device_runner', return_value=fresh_device_runner):
            with patch('model_services.device_worker.get_telemetry_client', Mock()):
                with patch('asyncio.run', Mock(return_value=None)):  # Mock asyncio.run to avoid coroutine issues
                    device_worker("worker_0", task_queue, result_queue, warmup_signals_queue, error_queue)
