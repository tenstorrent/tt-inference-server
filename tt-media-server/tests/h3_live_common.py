...
"""Shared machinery for the live MiniMax-H3 test modules (tests/test_minimax_h3_live*.py).

Configuration (H3_LIVE_* environment), the HTTP client, deployment control (fresh workers per
case, chip resets, poison tracking), reference assets, the request-combination table, the JSON
report, the on-disk evidence helpers and the contract checks (generate + DELETE, cancel, routing)
all live here so that every live module shares one implementation.  A test module imports the
fixtures it needs by name (pytest only discovers fixtures present in the module namespace):

    from h3_live_common import assets, deployment, report, served_task  # noqa: F401

``require_live()`` replaces the old module-level skip: call it at import time in every live module.
"""
...
def require_live() -> None:
    """Skip the calling module unless a live server is configured (call at import time)."""
    if not LIVE_URL:
        pytest.skip(
...
class Report:
    ...
    def dump(self) -> None:
        ...
        if os.environ.get("H3_LIVE_REPORT"):
            # several live modules share one run: append to what an earlier module wrote
            target = Path(os.environ["H3_LIVE_REPORT"])
            existing: list = []
            if target.exists():
                try:
                    ...
                    existing = []
            target.write_text(json.dumps(existing + self.rows, indent=1))
