...
| fl2va conditioner device hang, deterministic per INPUT (2026-09-06, metal 8fb0c4c0483, both quads): 1:1 one keyframe (594 presentation tokens), 1:1 two keyframes + long prompt (1205), 3:4 two keyframes + long prompt (1589) hang 2/2 in the Qwen3-VL text encoder (traceback in `model_qwen3vl.py` qkv_proj; all ranks `device timeout in fetch queue wait` after 300 s), while 4:3 long (same 1589 tokens, same 768-token vision block) and every 16:9 / 21:9 / 9:16 case pass | `live_shapes::TestFl2vaShapes::test_one_keyframe_aspect_ratios` (1:1 at 5 s), `::test_first_and_last_keyframes[1:1-long]`, `[3:4-long]` | FAIL while open (no xfail: a hang costs a chip reset); `H3_LIVE_SKIP_ASPECTS=1:1,3:4` keeps the rest of the canvas coverage runnable; `run_sequence` stops a sequence after a device hang |
