...
def test_run_merge_subprocess_respects_explicit_python_and_cwd(monkeypatch):
    ...
    amu.run_merge_subprocess(
        "b", "a", "o",
        python_executable="/x/py", cwd="/work", dtype_str="torch.float16",
    )
...
def test_cli_flags_from_launcher_parse_in_main(monkeypatch):
    ...
    monkeypatch.setattr(amu.subprocess, "run", _fake_run(store))
    amu.run_merge_subprocess(
        "base/model", "/adapters/a", "/out",
        python_executable="py", cwd="/w", dtype_str="torch.float32",
    )
    ...
    def fake_merge_adapter(
        ...
        called.update(
            base=base, adapter=adapter, output=output, dtype=dtype_str, verify=verify_load
        )
...
def test_cli_no_verify_load_flag(monkeypatch):
    ...
    monkeypatch.setattr(
        sys, "argv",
        ["prog", "--base-model", "b", "--adapter-path", "a", "--output-dir", "o",
         "--no-verify-load"],
    )
    amu.main()
