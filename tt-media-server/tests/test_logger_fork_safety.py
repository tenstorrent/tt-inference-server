...
Note: This deadlock is timing-dependent - it only occurs when fork()
happens while the lock is actually held. In production, this manifested
intermittently during device worker initialization.
"""
...
class TestDeadlockWithoutFix:
    ...
    Uses SIGALRM to detect the deadlock (child gets killed after timeout).
    """
    ...
    def test_child_deadlocks_without_lock_reset(self):
        script = "\n".join(
            [
                "import logging, os, signal, threading",
                "handler = logging.StreamHandler()",
                ...
                "    os._exit(0)",
                "_, status = os.waitpid(pid, 0)",
                "exit(0 if os.WIFSIGNALED(status) else 1)",
            ]
        )
        ...
        assert result.returncode == 0, (
            f"Expected child to deadlock and be killed by SIGALRM, "
            f"but subprocess exited {result.returncode}:\n"
