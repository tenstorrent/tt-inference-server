# #3 — Forge-TP release benchmark exceeds the 6h CI cap (conc-1 at batch-N)

Part of the forge-TP-p300x2 umbrella. Qwen3-32B release benchmark (run #5204) hit the **6h job
cap** and was cancelled; gemma will too once its tokenizer (#2) is fixed. Goal: a **small interim**
so CI doesn't hang QB2 for 6h, plus a pointer to the proper fix.

## Root cause
The release sweep (`benchmark_config._expand_text_sweep_params`) runs **both conc-1 and
conc-`max_concurrency`** for every isl/osl pair. At the batch-16 config, per-request rate is
wildly concurrency-dependent (single request still runs the full batch-16 graph):
...
| C. Evals-only in CI for now | workaround | zero bench cost | needs per-model workflow knob; no perf coverage |
| D. Raise tt-shield timeout to 8h | ✗ | avoids cancel only | **rejected** — still hangs QB2 8h |
| E. Engine: don't run full batch-N graph for 1 request | **proper fix** | removes the pathology | large (tt-xla/vllm_tt), long-term |
...
## Note
Independent of #2 (the tokenizer/vllm-uplift). #2 lets the benchmark *start*; #3 keeps it *bounded*.
