...
model_variant = os.environ.get("MODEL", "yolov8s")
...
YOLOV8_L1_SMALL_SIZE =  24576 #same for all variants

class YoloV8ModelError(Exception):
    """Base exception for YOLOv8 model errors"""
    pass

class InferenceError(YoloV8ModelError):
...
class TTYolov8Runner(BaseDeviceRunner):  
    def __init__(self, device_id: str):
        ...
        self.batch_size = 1  
        self.use_single_device = getattr(settings, "yolov8_use_single_device", True)
        self.image_manager = ImageManager(storage_dir="")
        self.model_variant = model_variant
        
        self.logger.info(f"Initializing YOLOv8 runner for variant: {self.model_variant}")
    ...
    def _get_device_params(self):
        """Get YOLOv8-specific device parameters."""
        return {
            "l1_small_size": YOLOV8_L1_SMALL_SIZE,
    ...
    def _mesh_device(self):
        """Create mesh device with YOLOv8-optimized configuration."""
        ...
        self.logger.info(
            f"Created mesh device with {device_count} {device_text} for YOLOv8"
        )
        return mesh_device
    ...
    def _create_model_location_generator(self, tt_metal_home: Path):
        """Create model location generator for YOLOv8 weights."""
    ...
    def _distribute_model(self) -> None:
        """Initialize and distribute YOLOv8 model on device."""
        try:
            ...
            self.logger.info("YOLOv8PerformantRunner created successfully")
            
        except Exception as e:
            import traceback
            self.logger.error(f"Model distribution failed at step: {e}")
            self.logger.error(f"Full traceback: {traceback.format_exc()}")
            raise RuntimeError(f"Failed to initialize YOLOv8PerformantRunner") from e
    ...
    async def load_model(self, device) -> bool:
        """Load YOLOv8 model with weights and distribute on device."""
        ...
        if os.environ.get("MODEL_WEIGHTS_PATH"):
            weights_dir = Path(os.environ["MODEL_WEIGHTS_PATH"])
            pt_file = weights_dir / f"{self.model_variant}.pt"
            ...
            if pt_file.exists() and not pth_file.exists():
                ...
                yolov8_ckpt = torch.load(pt_file, weights_only=False)
                state_dict = yolov8_ckpt["model"].float().state_dict()
                torch.save(state_dict, pth_file)
                self.logger.info(f"Created .pth state dict at {pth_file}")
            
            if pth_file.exists():
                os.environ["MODEL_WEIGHTS_PATH"] = str(temp_weights_dir)
        ...
        try:
            ...
            self.logger.warning(f"Warmup failed or skipped: {e}")
        return True
            ...
            try:
                if not eval_mode:
                    ...
                    if isinstance(raw_output, (list, tuple)):
                        try:
                            ...
                            # Other elements are intermediate outputs not needed for postprocessing
                            torch_output = ttnn.to_torch(raw_output[0], dtype=torch.float32, mesh_composer=self.outputs_mesh_composer)
                        except:
                    ...
                    if isinstance(raw_output, (list, tuple)):
                        try:
                            if hasattr(raw_output[0], 'cpu'):
                                cpu_item = raw_output[0].cpu()
                                if hasattr(cpu_item, 'to_torch'):
                                    converted_tensor = cpu_item.to_torch()
                                else:
                ...
                for i, result in enumerate(results_batch):
                    if i < len(batch_images):  # Only process actual images, not padding
                        formatted_detections = self._format_yolov8_detections(result, eval_mode)
    ...
    def _format_yolov8_detections(self, result, eval_mode=False):
        """Format YOLOv8 result into expected format."""
        detections = []
        
        # Handle both ultralytics Results objects and dict format
        if hasattr(result, 'boxes') and hasattr(result.boxes, 'xyxy'):
            # Result is an ultralytics Results object
            xyxy = result.boxes.xyxy
            conf = result.boxes.conf if hasattr(result.boxes, 'conf') else torch.ones(len(xyxy))
            cls = result.boxes.cls if hasattr(result.boxes, 'cls') else torch.zeros(len(xyxy))
