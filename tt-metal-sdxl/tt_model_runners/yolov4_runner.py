...
from models.demos.yolov4.common import (
    get_mesh_mappers,
)  # Use models.demos.utils.common_demo_utils for tt-metal commit v0.63+
...
DEFAULT_CONFIDENCE_THRESHOLD = (
    0.01  # Low threshold for COCO evaluation - let evaluator handle precision/recall
)
...
class TTYolov4Runner(BaseDeviceRunner):
    ...
    def _log_device_configuration(self):
        ...
        if self.use_single_device:
            ...
            self.logger.warning(
                "YOLOv4 using multi-device operation (may cause memory issues)"
            )
    ...
    def _mesh_device(self):
        ...
        self.logger.info(
            f"Created mesh device with {device_count} {device_text} for YOLOv4"
        )
        return mesh_device
