...
PAYLOAD = {"prompt": "Michael Jordan blocked by Spud Webb", "num_inference_step": 5}
...
def check_api():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return f"http_{response.status_code}", elapsed
    except Exception as e:
        elapsed = time.time() - start_time
        return f"error_{type(e).__name__}", elapsed
...
def main():
    ...
    for i in range(num_run_times):
        ...
        print(
            f"{timestamp} - Run {i+1}/{num_run_times} - "
            f"{status} time: {elapsed:.2f}s"
        )
