...
def generate_speech_streaming(
    text: str, speaker_id: int = 0, output_file: str = "output_streaming.wav"
):
    """
    ...
    payload = {"text": text, "stream": True, "speaker_id": speaker_id}
...
def generate_speech_non_streaming(
    text: str, speaker_id: int = 0, output_file: str = "output_non_streaming.wav"
):
    """
...
Examples:
  ...
  python tts_streaming_example.py "Test" --speaker-id 100
    ...
    parser.add_argument(
        "-s", "--speaker-id", type=int, default=0, help="Speaker voice ID (default: 0)"
    )
    parser.add_argument(
        "--url",
        type=str,
    ...
    if args.compare:
        ...
        success_non_stream, time_non_stream, size_non_stream = (
            generate_speech_non_streaming(
        ...
        if success_non_stream:
            ...
            if time_stream < time_non_stream:
                ...
                print(
                    f"  Note: Streaming took {diff:.3f}s longer (still useful for real-time applications)"
                )
