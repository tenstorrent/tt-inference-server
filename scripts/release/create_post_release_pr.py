...
from --token or, failing that, the env vars TMP_ACVEJICTT_PAT / GH_PAT /
...
TOKEN_ENV_VARS = ("TMP_ACVEJICTT_PAT", "GH_PAT", "GITHUB_TOKEN")
UNKNOWN = "UNKNOWN"
...
def main() -> None:
    ...
    ap.add_argument(
        "--token",
        default=None,
        help="Token for tt-shield reads (default: env TMP_ACVEJICTT_PAT/GH_PAT/GITHUB_TOKEN)",
    )
    ap.add_argument(
    ...
    if args.tt_shield_run_id:
        ...
        if token:
            ...
            print(
                "WARNING: no token (TMP_ACVEJICTT_PAT/GH_PAT/GITHUB_TOKEN); "
                "CI Job Link cells will be UNKNOWN.",
                file=sys.stderr,
            )
    else:
