...
def get_device_status_for_template(
    ...
    for dev_spec in template.device_model_specs:
        if dev_spec.device == device:
            return template.status.name if hasattr(template.status, 'name') else str(template.status)
    return "-"
...
def generate_model_page(model_name: str, templates: List[ModelSpecTemplate]) -> str:
    ...
    device_sections.sort(
        key=lambda x: device_order.index(x[0].device) if x[0].device in device_order else 999
    )
    ...
    for dev_spec, template in device_sections:
        ...
        template_hf_weights = template.weights[0] if template.weights else None
        if template_hf_weights:
            template_hf_url = f"https://huggingface.co/{template_hf_weights}"
            lines.append(f"HF Repo: [{model_name}]({template_hf_url})")
            lines.append("")
        ...
        # Prerequisites link
        lines.append("[Prerequisites](../../prerequisites.md)")
        lines.append("")
        
        # Deploy section
        lines.append("#### Deploy Inference Server")
        lines.append("")
        ...
        lines.append("```bash")
        lines.append(f"python3 run.py --model {model_name} --device {device_arg} --workflow server --docker-server")
        lines.append("```")
        lines.append("")
        ...
        lines.append(f"| Implementation Code | [{template.impl.impl_name}]({code_link}) |")
...
def generate_model_type_table(
    ...
    for model_name, model_templates in model_groups.items():
        has_supported = any(t.status in SUPPORTED_STATUS_GROUPS for t in model_templates)
        has_experimental = any(t.status == ModelStatusTypes.EXPERIMENTAL for t in model_templates)
...
def generate_model_type_page(
    ...
    lines.append("")
    lines.append(f"This page lists all supported {full_name.lower()}s and their device compatibility.")
    lines.append("")
    ...
    supported_table = generate_model_type_table(templates, model_type, devices, is_experimental=False)
    if supported_table:
    ...
    experimental_table = generate_model_type_table(templates, model_type, devices, is_experimental=True)
    if experimental_table:
        lines.append("## Experimental Models")
        lines.append("")
        lines.append("Models with EXPERIMENTAL status are under active development and may have stability or performance issues.")
        lines.append("")
...
def generate_directory_readme(templates: List[ModelSpecTemplate]) -> str:
    ...
    lines.append("")
    lines.append("This directory contains documentation for all models supported on Tenstorrent hardware.")
    lines.append("")
    ...
    for device in device_order:
        ...
        lines.append(f"- [{product_name} ({device.name})](models_by_hardware.md#{anchor})")
