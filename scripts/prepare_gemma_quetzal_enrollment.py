...
def _current_repository_revision(repo_root: Path, field: str) -> str:
    try:
        ...
        raise EnrollmentError(
            f"cannot resolve the exact {field} checkout revision"
        ) from error
...
def validate_evidence(
    ...
    _need(
        identity.get("tt_metal_patchset_sha256"),
        PATCHSET,
        "identity.tt_metal_patchset_sha256",
    )
    _need(
        identity.get("patchset_applied_manifest_matches"),
        True,
    ...
    if not isinstance(package_id, str) or not PACKAGE.fullmatch(package_id):
        raise EnrollmentError(
            "package_id must be a complete content-addressed package id"
        )
    ...
    if (
        PurePosixPath(host_root).name != package_id
        or PurePosixPath(container_root).name != package_id
    ):
        raise EnrollmentError(
    ...
    if not isinstance(pcc, (int, float)) or isinstance(pcc, bool) or pcc < 0.99:
        raise EnrollmentError(
            "qualification.pcc must be a fresh exact-package result >= 0.99"
        )
    ...
    _need(
        ...
        True,
        "qualification.zero_device_holders_after",
    )
    _need(
...
def render_fragments(
    ...
    env = {
        ...
        "QUETZAL_GENERATOR_SOURCE_REVISION": ARTIFACT_SOURCE,
        "QUETZAL_PACKAGE_ID": exact["package_id"],
        "QUETZAL_BUNDLE_MANIFEST_SHA256": exact["manifest_sha"],
        "QUETZAL_REQUIRED_TT_METAL_COMMIT": TT_METAL,
        ...
        "QUETZAL_REQUIRED_PREFILL_BUCKETS": "4096",
        "QZ_MMAP_WEIGHTS": "1",
    ...
    implementation = {
        ...
        "ci": {
            schedule: {
                "devices": ["P300X2"],
                "device-args": {"P300X2": {"additional-args": args}},
            }
    ...
    if any(row.get("impl") == "quetzal" for row in rows):
        raise EnrollmentError(
            "active config already has a Gemma Quetzal row; refuse duplicate promotion"
        )
...
def main() -> int:
    ...
    (args.out_dir / "gemma-dev-catalogue-entry.yaml").write_text(
        yaml.safe_dump(rendered["catalogue"], sort_keys=False)
    )
    for name, key in (
        ("gemma-models-ci-implementation.json", "implementation"),
