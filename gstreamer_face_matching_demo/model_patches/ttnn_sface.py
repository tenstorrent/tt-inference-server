...
class TtSFace:
    ...
    def load_weights_from_torch(self, torch_model, weights_mesh_mapper=None):
        """Load weights from PyTorch SFace model."""

        def to_ttnn(w, b):
            return Conv2dConfiguration.convert_torch_weight_and_bias_to_ttnn(w, b, mesh_mapper=weights_mesh_mapper)
...
def create_sface_model(device, torch_model=None, weights_mesh_mapper=None) -> TtSFace:
    """Factory function to create SFace model."""
    model = TtSFace(device)
    if torch_model is not None:
        model.load_weights_from_torch(torch_model, weights_mesh_mapper=weights_mesh_mapper)
    return model
