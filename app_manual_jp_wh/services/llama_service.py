...
class LlamaService:
    """ELYZA-JP-8B (Japanese) on TT Metal Device 0 (WH N150)."""
    ...
    def format_prompt(self, user_message: str, conversation_history: list = None, system_prompt: str = None) -> str:
        ...
        if system_prompt is None:
            system_prompt = (
                ...
                "ユーザーが英語で話しかけても、必ず日本語のみで回答してください。英語では絶対に回答しないでください。"
                "音声アシスタントとして、簡潔で分かりやすい回答をしてください。"
...
"""
    ...
    async def warmup(self):
        ...
        try:
            ...
            os.environ.setdefault('HF_MODEL', 'elyza/Llama-3-ELYZA-JP-8B')
            os.environ.setdefault(
                'TT_MESH_GRAPH_DESC_PATH',
                '/home/container_app_user/tt-metal/tt_metal/fabric/mesh_graph_descriptors/n150_mesh_graph_descriptor.textproto',
            )
    ...
    async def generate_response_streaming(self, message: str, max_tokens: int = 256, conversation_history: list = None, system_prompt: str = None):
        """Async generator that yields sentences as they complete during LLM token generation.

        Yields dict with:
          - type: 'sentence' | 'done' | 'error'
