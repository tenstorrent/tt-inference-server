...
class MLP(LightweightModule):
    ...
    def __init__(
        ...
        # Cap to this chip's compute grid (P150 is 13x10; Wormhole N150 is 8x8).
        _cg = device.compute_with_storage_grid_size()
        rows_gu, cols_gu = _pick_grid_for_in0_block_w2(
            k_tiles_gu, n_tiles_gu, max_rows=_cg.y, max_cols=_cg.x
        )
        ...
        rows_d, cols_d = _pick_grid_for_in0_block_w2(
            k_tiles_d, n_tiles_d, max_rows=_cg.y, max_cols=_cg.x
        )
    ...
    def forward(self, x: ttnn.Tensor, mode: str = "prefill") -> ttnn.Tensor:
        ...
        if is_decode and seq_len < 1024:
            ...
            # Gate/up and down can pick different core counts (WH 8x8 vs BH 13x10).
            # Reshard when the mul output layout does not match down's in0 spec.
            if hidden_sharded.memory_config() != self._decode_down_in0_memcfg:
                hidden_for_down = ttnn.to_memory_config(hidden_sharded, self._decode_down_in0_memcfg)
                ttnn.deallocate(hidden_sharded)
                hidden_sharded = hidden_for_down
