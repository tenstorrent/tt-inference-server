#!/bin/bash
# TT-HOME Voice Assistant - Blackhole QB2 (Japanese, ELYZA-JP)
# Usage: ./start.sh
#
# ===== Architecture =====
# Hardware: Blackhole QB2 (4x P150 chips)
# Docker: Lean base image (no weights) + weights mounted from host
#
# Device allocation:
#   ELYZA-JP-8B:         P150 (1 chip, device 0)
#   Whisper large-v3:    P150 (1 chip, device 1) - Japanese ASR
#   Qwen3 TTS (GUEST):  P150 (1 chip, device 2) - Jim voice
#   Qwen3 TTS (HOST):   P150 (1 chip, device 3) - Riata voice (podcast)
#
# Prerequisites:
#   1. Docker image: ghcr.io/tenstorrent/tt-inference-server/tt-voice-assistant:manual_jp_tt-home
#      (lean: tt-metal + packages + ELYZA tensor cache + TT JIT kernel cache; NO model weights)
#   2. Model weights downloaded to $WEIGHTS_DIR/hf_cache/
#      - elyza/Llama-3-ELYZA-JP-8B
#      - Qwen/Qwen3-TTS-12Hz-1.7B-Base
#      - openai/whisper-large-v3
#   See ../TT-HOME-README.md for full setup, debugging, and image-build notes.
# ==============================

set -e

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
CONTAINER="tt-bh-qb2-elyza-jp-manual"
IMAGE="${IMAGE:-ghcr.io/tenstorrent/tt-inference-server/tt-voice-assistant:manual_jp_tt-home}"
LOCAL_APP="$SCRIPT_DIR"

# Host path to model weights (HF cache + tensor cache)
...
echo "=========================================="
echo "  TT-HOME Voice Assistant (Blackhole QB2 - ELYZA Japan)"
...
echo "       Whisper large-v3 (Device $WHISPER_DEVICES, P150, multilingual)..."
...
echo "    Device 1: Whisper large-v3 (P150, multilingual/Japanese)"
