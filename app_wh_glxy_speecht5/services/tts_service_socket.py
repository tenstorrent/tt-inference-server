...
def _clean_for_tts(text: str) -> str:
    """Strip markdown/formatting that SpeechT5 can't handle, keep spoken words only."""
    t = text
    t = re.sub(r'\*\*([^*]+)\*\*', r'\1', t)
    t = re.sub(r'\*([^*]+)\*', r'\1', t)
    t = re.sub(r'#{1,6}\s*', '', t)
...
class TTSService:
    ...
    def _send_request_to(self, sock_path: str, request: dict) -> dict:
        """Send a request to a specific TTS server socket and get response."""
    ...
    async def warmup(self):
        """Warmup / check if TTS server is ready."""
    ...
    async def check_server(self):
        ...
        try:
            ...
            if response.get("status") == "ok":
                ...
                logger.info(f"✅ TTS server is ready!")
            else:
    ...
    def _restart_server(self):
        ...
        try:
            subprocess.run(['pkill', '-f', 'speecht5_ttnn_server.py --speaker-id 7306'], timeout=5)
            import time
            time.sleep(2)
            subprocess.Popen([
                'bash', '-c',
                ...
                'python /home/container_app_user/voice-assistant/servers/speecht5_ttnn_server.py '
                '--speaker-id 7306 --socket /tmp/tts_server.sock >> /tmp/tts_server.log 2>&1'
            ])
            time.sleep(30)
            self._consecutive_failures = 0
            self.restarting = False
