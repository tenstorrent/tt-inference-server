...
Under concurrent load (e.g. 32 concurrent streaming requests), a forge single-chip LLM served via tt-media-server **intermittently wedges**: the vLLM EngineCore stops producing tokens entirely (0 output), streaming requests sit in-flight until the client times out, and the server becomes unresponsive. It is **not** an OOM, **not** a compile, and **not** a config problem. The host enqueues a device→host readback and **blocks in `FDMeshCommandQueue::read_completion_queue`** waiting for a completion that never arrives. **Caveat on attribution:** that frame is only where it is observed *blocking* — the root cause is not confirmed. It may live in tt-metal fast-dispatch, in tt-xla's concurrent driving of these `from_device` readbacks, or in their interaction; treat "tt-metal FD completion queue" as a lead, not a conclusion. Reproduced live (multiple times) on a warm, healthy Qwen3-8B server with a clean backtrace (below), on the **shipping chunk-1024 / gmu-0.35 production config**. It is an upstream runtime bug (tt-metal / tt-xla layer); not fixable by chunk size or GPU_MEMORY_UTILIZATION.
...
### Pure-tt-xla repro (no tt-media-server) — recommended, not yet achieved
A stock-vLLM / standalone reproducer (no tt-media-server) **should be built** — it is the artifact the tt-xla/tt-metal team needs. It was **not** achieved here: stock `vllm serve` and in-process `AsyncLLMEngine` (synchronized waves and bursty continuous, up to 20k requests) did not trip it. Starting scaffolding is in `~/tt-xla/HANG_4521_REPRO_README.md` (`serve_forge_qwen3_8b.sh`, `tt_xla_hang_repro.py` with `HANG_DRIVE=continuous`/`HANG_SKIP_WARMUP=1`). The most promising untested angle: **concurrent compile + execute** — a new graph shape compiling while other requests run concurrently (which is exactly what tt-media-server's first conc=64 batch does, and where it reliably hangs). If that also fails to reproduce standalone, the trigger is likely something specific to tt-media-server's multiprocessing worker / IPC path, and reducing *that* to a minimal harness is the next step.

### Recommended next steps for the assignee
- **Don't over-anchor on tt-metal fast-dispatch.** The `read_completion_queue` / `copy_completion_queue_data_into_user_space` frame is where it blocks, but the cause could be tt-xla's concurrent `from_device` driving (or the compile/dispatch interleave) rather than the FD queue itself. Investigate both the tt-metal side (can a completion be dropped/lost or deadlock under many concurrent device→host readbacks?) **and** the tt-xla side (how concurrent input-relayout `from_device` calls are issued/serialized).
- **Build a minimal reproducer** driven by the pattern that triggers it: **bursty, overlapping concurrent `enqueue_read`/`from_device`** (many admitted near-simultaneously), ideally with a graph being programmed/compiled at the same time — matching tt-media-server's `device_worker_dynamic_batch` (`get_many(32)` → burst of `generate()` → keep pulling). Smooth one-at-a-time admission (vLLM-native) does NOT trip it.
...
## Where it blocks (captured live via gdb) — observed frame, not confirmed root cause
