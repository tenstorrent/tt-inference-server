...
Usage: $0 [options]
  ...
  --no-worker                  etcd + frontend only (for cpp_server integration tests
                               that register their own DynamoWorkerServer in-process)
...
if [[ "$WORKER_ENABLED" == "0" && "$ROUTING_MODE_COUNT" -gt 0 ]]; then
    die "--no-worker is incompatible with --prefill-direct / --prefill-gateway / --dynamo-routing"
fi
...
if [[ "$WORKER_ENABLED" == "0" ]]; then
    # Host networking: integration tests (often in a sibling container) reach
    # published services via the docker bridge gateway. The frontend must also
    # dial back to the test process's advertised Dynamo TCP address — bridge
    # isolation blocks that unless frontend/etcd share the host netns.
