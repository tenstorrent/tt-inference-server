...
docker run -d --name "$FRONTEND_NAME" --network "$NETWORK_NAME" -p "${FRONTEND_HOST_PORT}:8000" \
    ...
    -e TT_NATIVE_PREFILL_POLICY="${TT_NATIVE_PREFILL_POLICY:-threshold}" \
    -e TT_PREFILL_ON_DECODE_MAX_TOKENS="${TT_PREFILL_ON_DECODE_MAX_TOKENS:-${MAX_TOKENS_TO_PREFILL_ON_DECODE:-1000}}" \
    -e RUST_LOG="${RUST_LOG:-}" \
