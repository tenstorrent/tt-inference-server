"""
Mode-specific system prompts for the LLM (Gemma4-12B).

Each mode gives the model a different "personality" and instruction set.
The prompt is injected as the system message in the chat template.
"""
...
MODES = {
    ...
    "docs": {
        ...
        "system_prompt": (
            ...
            "Your response will be read aloud, so use plain spoken sentences only. "
            "Do NOT use markdown, headings, bold, asterisks, bullets, or numbered lists with symbols. "
            "Prefer a short paragraph summary, then say 'Key points:' followed by "
            "three or four complete sentences, each starting with First, Second, Third, Fourth. "
            "Always finish every point completely — never stop mid-sentence. "
