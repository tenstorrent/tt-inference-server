...
_agentic_traces_config_list: List[AgenticTracesConfig] = [
    ...
    # GPT-OSS-120B on WH Galaxy. Onboarded from the hand-validated KIX1 run
    # (report_openai__gpt-oss-120b_2026-09-08T130311+0000): the Kimi Claude-Code
    # SWE-bench scenario replays cleanly against gpt-oss on the default faithful
    # history mode (the recorded assistant turns are re-generated by gpt-oss).
    # max_context_length is left None -> derives 131072 from the galaxy
    # ModelSpec; the driver passes it with --no-resolve-model-context because
    # the Console /v1/models does not report the window.
    AgenticTracesConfig(
        model_id="id_gpt-oss_gpt-oss-120b_galaxy",
        inferencex_git_ref="ddeb02eb9c5c89f44e2e4950e741b499d0b8190a",
        runs=(
            # AgentX (InferenceX / AIPerf) synthetic Weka coding traces at the
            # galaxy max_concurrency. NOTE: the _256k dataset carries contexts
            # above the 131072 window, so some traces overflow; if the run's
            # context_overflow exceeds ~1% the AgentX submission is flagged
            # invalid (the numbers still render). Swap to a <=128k dataset
            # variant for a clean AgentX verdict.
            AgenticTracesRunSpec(
                trace_source=TraceSource.INFERENCEX_AGENTX,
                public_dataset="semianalysis_cc_traces_weka_062126_256k",
                concurrency=32,
            ),
            # SwarmOne swo-bench: the Kimi Claude-Code SWE-bench scenario. FULL
            # replays all tasks at concurrency 8; CI replays only the short
            # sympy-bugfix task at concurrency 1 (the shape validated on KIX1).
            AgenticTracesRunSpec(
                trace_source=TraceSource.SWARMONE,
                scenario="claude-code-swe-bench-python-kimi-k2.7-code",
                public_dataset="",
