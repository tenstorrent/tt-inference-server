...
ttl 1.1.1 API notes:
  - reduce_sum(input, scaler_block, *, dims)  — scaler is a TensorBlock
  - broadcast(input, output_block, *, dims)   — dims=[-1] expands columns
  - No shape= argument on reduce_sum; no ttl.block.broadcast
"""
        ...
        def compute():
            with x_dfb.wait() as xv, sc_dfb.wait() as sc, w_dfb.wait() as wv:
                ...
                with sq_dfb.reserve() as sq:
                    sq.store(xv * xv)
                # mean(x^2) = reduce_sum(x^2, 1/hidden_dim, dims=[-1])
                # reduce along hidden (last) dimension; scaler provides 1/hidden
                with sq_dfb.wait() as sqv, sum_dfb.reserve() as sm:
                    sm.store(ttl.math.reduce_sum(sqv, sc, dims=[-1]))
                # 1 / sqrt(mean(x^2))
