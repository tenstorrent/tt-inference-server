    ...
    dev_dir, prod_dir = _write_catalogs(
        ...
        dev_llm="""
            ...
            - weights: [Qwen/Qwen3.6-27B]
              ...
              inference_engine: VLLM
              device_model_specs:
                - {device: P300X2, max_concurrency: 1, max_context: 8192, default_impl: true}
            - weights: [Qwen/Qwen3.6-27B]
              impl: quetzal
              inference_engine: VLLM
              device_model_specs:
...
def test_quetzal_promotion_requires_digest_pinned_package_image(tmp_path):
    ci_path, dev_dir, prod_dir = _quetzal_fixture(tmp_path)
    before = (prod_dir / "llm.yaml").read_bytes()

    with pytest.raises(ValueError, match="explicit immutable package-capable image"):
...
def test_quetzal_image_requires_canonical_immutable_digest(image):
    with pytest.raises(ValueError, match="immutable.*sha256"):
        validate_digest_pinned_image(image)


def test_quetzal_promotion_selects_non_default_leaf_and_injects_image(tmp_path):
    ci_path, dev_dir, prod_dir = _quetzal_fixture(tmp_path)
    ...
    identity = ("Qwen/Qwen3.6-27B", "P300X2", "vLLM", "quetzal")
    assert report["requested_identities"] == (identity,)
    leaves = _load_leaf_templates(prod_dir)
    assert set(leaves) == {identity}
    assert leaves[identity]["docker_image"] == QUETZAL_IMAGE
    assert leaves[identity]["device_model_specs"][0]["default_impl"] is False


def test_real_quetzal_candidates_promote_without_losing_package_identity(tmp_path):
    import shutil
    ...
    prod_dir = tmp_path / "prod"
    shutil.copytree(promote_module.DEFAULT_PROD_DIR, prod_dir)

    report = promote(
        ci_path,
        promote_module.DEFAULT_DEV_DIR,
        prod_dir,
        **PINS,
        quetzal_docker_image=QUETZAL_IMAGE,
    )
    ...
    assert set(report["requested_identities"]) == {
        (weight, "P300X2", "vLLM", "quetzal") for weight in expected
    }
    leaves = _load_leaf_templates(prod_dir)
    for weight, (package_id, manifest_sha) in expected.items():
        leaf = leaves[(weight, "P300X2", "vLLM", "quetzal")]
        env = leaf["device_model_specs"][0]["env_vars"]
        assert leaf["docker_image"] == QUETZAL_IMAGE
        assert env["QUETZAL_PACKAGE_ID"] == package_id
        assert env["QUETZAL_BUNDLE_MANIFEST_SHA256"] == manifest_sha
        assert all(
            "/mnt/" not in value and "/home/ttuser/" not in value
