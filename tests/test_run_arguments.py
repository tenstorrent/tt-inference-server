...
class TestOverrideArgsIntegration:
    ...
    def test_docker_server_passes_model_spec_env_vars(self, mock_setup_config):
        """Test that run_docker_server passes model_spec.env_vars to the Docker container."""
        from pathlib import Path

        # Create mock model_spec with environment variables
        mock_model_spec = MagicMock()
        mock_model_spec.model_id = "test-model-id"
        mock_model_spec.device_type = "t3k"
        mock_model_spec.docker_image = "test:image"
        mock_model_spec.impl.impl_name = "tt-transformers"
        mock_model_spec.hf_model_repo = "Qwen/Qwen2.5-72B-Instruct"
        mock_model_spec.subdevice_type = None
        mock_model_spec.env_vars = {
            "VLLM_ALLOW_LONG_MAX_MODEL_LEN": "1",
            "VLLM_CONFIGURE_LOGGING": "1",
            "CUSTOM_ENV_VAR": "test_value"
        }

        # Create cli_args
        mock_cli_args = MagicMock()
        mock_cli_args.model = "Qwen2.5-72B-Instruct"
        mock_cli_args.device = "t3k"
        mock_cli_args.workflow = "server"
        mock_cli_args.service_port = "8000"
        mock_cli_args.interactive = False
        mock_cli_args.dev_mode = False
        ...
        with patch(
            ...
            # Check that model spec environment variables are passed to Docker
            env_vars_found = {}
            for i, arg in enumerate(docker_command):
                if arg == "-e" and i + 1 < len(docker_command):
                    env_setting = docker_command[i + 1]
            ...
            # Verify that our model spec environment variables are included
            assert "VLLM_ALLOW_LONG_MAX_MODEL_LEN" in env_vars_found, f"VLLM_ALLOW_LONG_MAX_MODEL_LEN not found in docker command env vars: {env_vars_found}"
            assert env_vars_found["VLLM_ALLOW_LONG_MAX_MODEL_LEN"] == "1", f"VLLM_ALLOW_LONG_MAX_MODEL_LEN has wrong value: {env_vars_found['VLLM_ALLOW_LONG_MAX_MODEL_LEN']}"
            assert "VLLM_CONFIGURE_LOGGING" in env_vars_found, f"VLLM_CONFIGURE_LOGGING not found in docker command env vars: {env_vars_found}"
            assert "CUSTOM_ENV_VAR" in env_vars_found, f"CUSTOM_ENV_VAR not found in docker command env vars: {env_vars_found}"
            assert env_vars_found["CUSTOM_ENV_VAR"] == "test_value", f"CUSTOM_ENV_VAR has wrong value: {env_vars_found['CUSTOM_ENV_VAR']}"
