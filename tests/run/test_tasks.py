...
class TestTask:
    ...
    def _generate_multiple_mode_params(self) -> List[Dict]:
        ...
        # Create TestParamSpace with impl if available
        if hasattr(self.test_args, "impl"):
            param_space = TestParamSpace(self.env_vars["MODEL_NAME"], self.env_vars["MESH_DEVICE"], self.test_args.impl)
        else:
            param_space = TestParamSpace(self.env_vars["MODEL_NAME"], self.env_vars["MESH_DEVICE"])
