...
pytestmark = pytest.mark.skipif(
    M.ffmpeg_binary() is None or shutil.which("ffprobe") is None,
    reason="the mock encodes real clips with ffmpeg and the judge probes them with ffprobe",
)
...
def test_t2va_plan_passes_and_every_clip_is_judged(mock, pack, tmp_path, monkeypatch):
    ...
    rows = [json.loads(line) for line in open(result["artifacts"]["results_jsonl"])]
    ...
    assert (
        open(tmp_path / "out" / f"smoke_status_{result['combo']}_t2va.log")
        .readline()
        .strip()
        == "ok"
    )
...
def test_resume_skips_measured_rows_already_present(mock, pack, tmp_path, monkeypatch):
    ...
    rows = [json.loads(line) for line in open(second["artifacts"]["results_jsonl"])]
