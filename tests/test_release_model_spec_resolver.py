...
def test_collect_release_combos_preserves_explicit_implementation():
    ci_config = {
        "models": {
            "model-a": {
                "implementations": [
...
def test_resolve_release_combo_honors_non_default_implementation(tmp_path):
    dev_dir = tmp_path / "dev"
    dev_dir.mkdir()
    (dev_dir / "llm.yaml").write_text(
        textwrap.dedent(
            ...
            - weights: [Qwen/Qwen3.6-27B]
              ...
              inference_engine: VLLM
              device_model_specs:
                - {device: P300X2, max_concurrency: 1, max_context: 8192, default_impl: false}
            """
        )
    )
    for filename in MODEL_SPEC_CATALOG_FILES:
    ...
    resolved = resolve_release_combo(
        ReleaseCombo(
            "Qwen3.6-27B",
            InferenceEngine.VLLM,
            DeviceTypes.P300X2,
