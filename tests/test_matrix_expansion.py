...
class TestAllSuitesLoad:
    ...
    IMAGE models are fully onboarded to the v2 engine (routed by model_type in
    workflows/v2_bridge.py), so there is no v1 image.json suite; image matrix
    coverage lives in tt-inference-server-v2/tests/test_module/test_matrix_expansion.py.
    """

    def test_total_suite_count(self):
        all_suites = load_suite_files()
        assert len(all_suites) == 15
