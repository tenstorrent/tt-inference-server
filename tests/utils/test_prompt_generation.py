...
def template_path():
    with tempfile.NamedTemporaryFile(mode='w', suffix='.j2', delete=False) as temp:
        temp.write("""{% for message in chat_history %}{% if message.role == 'user' %}User: {{ message.content }}{% endif %}{% endfor %}""")
        temp_path = temp.name
    yield temp_path
    os.unlink(temp_path)
...
def test_alpaca_eval_dataset(tokenizer_model):
    ...
    assert all(isinstance(p, str) for p in prompts)
    assert all(l <= 51 for l in lengths)  # 50 + 1 for vLLM token
...
def test_with_template(tokenizer_model, template_path):
    ...
    assert len(prompts) == 2
    assert all(isinstance(p, str) for p in prompts)
    assert all(l <= 31 for l in lengths)  # 30 + 1 for vLLM token
...
def test_very_short_sequence(tokenizer_model):
    ...
    prompts, lengths = generate_prompts(args)
    assert len(prompts) == 2
    assert all(l <= 2 for l in lengths)  # 1 + 1 for vLLM token
