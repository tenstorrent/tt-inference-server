...
class TestTTSServerHealth:
    ...
    def test_server_is_running(self):
        ...
        try:
            response = requests.get(f"{BASE_URL}/tt-liveness", headers=HEADERS, timeout=5)
            assert response.status_code == 200, f"Server not accessible: {response.status_code}"
        except requests.exceptions.ConnectionError:
            pytest.fail("Server is not running. Start it with: uvicorn main:app --lifespan on --port 8000")
...
class TestTTSNonStreaming:
    ...
    def test_simple_text_generation(self):
        ...
        with wave.open(io.BytesIO(audio_data)) as wav:
            assert wav.getframerate() == EXPECTED_SAMPLE_RATE, f"Expected {EXPECTED_SAMPLE_RATE}Hz, got {wav.getframerate()}Hz"
            assert wav.getnchannels() == EXPECTED_CHANNELS, f"Expected {EXPECTED_CHANNELS} channel(s), got {wav.getnchannels()}"
            assert wav.getsampwidth() == EXPECTED_BIT_DEPTH // 8, f"Expected {EXPECTED_BIT_DEPTH}-bit, got {wav.getsampwidth() * 8}-bit"
...
class TestTTSAudioQuality:
    ...
    def test_audio_not_silent(self):
        ...
        with wave.open(io.BytesIO(response.content)) as wav:
            ...
            assert silence_ratio < 0.9, f"Audio appears to be {silence_ratio*100}% silent"
