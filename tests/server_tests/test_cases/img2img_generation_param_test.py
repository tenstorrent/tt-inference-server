...
def load_test_image():
    """Load test image from payload file."""
    payload_path = TEST_PAYLOADS_PATH / "image_client_img2img_payload"
    try:
...
def create_payload(image: str, guidance_scale: float = GUIDANCE_SCALE) -> dict:
    """Create a payload for img2img request with given parameters."""
    return {
        "prompt": PROMPT,
        "image": image,
...
class Img2ImgGenerationParamTest(BaseTest):
    ...
    async def _run_specific_test_async(self):
        self.url = f"http://localhost:{self.service_port}/image/image-to-image"
        ...
        if not test_image:
            return {
                "success": False,
                "error": "Failed to load test image from payload file",
            }

        # Create payloads: default, same (for consistency), different guidance_scale
        payloads = [
            create_payload(test_image, GUIDANCE_SCALE),
            create_payload(test_image, GUIDANCE_SCALE),  # Same for consistency check
            create_payload(test_image, GUIDANCE_SCALE_CHANGED),
        ]
    ...
    async def _run_concurrent_requests(self, payloads: list[dict]) -> list[dict]:
        """
        Run concurrent img2img requests with warmup.
