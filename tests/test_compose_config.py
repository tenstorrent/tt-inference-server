...
class TestLookupContract:
    ...
    def test_media_engine_isolated_from_vllm(self, contracts_yaml):
        path = lookup_contract(
            "media", Version("0.11.0"), contracts_path=contracts_yaml
        )
...
class TestProductionContractsYaml:
    ...
    def test_vllm_pre_0_11_has_no_contract(self):
        # Pre-0.11 vLLM is intentionally absent from production contracts.yml
        # — workflows.validate_setup refuses it earlier with a migration
        # message. If someone bypasses that and reaches lookup_contract,
        # it must fail rather than silently picking the modern contract.
        from workflows.workflow_types import InferenceEngine

        with pytest.raises(NoMatchingContractError):
            lookup_contract(InferenceEngine.VLLM.value, Version("0.10.1"))
...
class TestBuildComposeCommand:
    ...
    def test_all_overlays_stacking_order(self, tmp_path):
        ...
        files, env = build_compose_command(
            ms, rc, setup_config=sc, json_fpath=json_file
        )
    ...
    # ------------------------------------------------------------------
    # 7. Pre-0.11 image tag — refused before reaching build_compose_command;
    #     if it does reach, NoMatchingContractError fires.
    # ------------------------------------------------------------------
    def test_pre_0_11_image_has_no_contract(self):
        ms = _model_spec(image_tag="0.10.4-abc")
        rc = _runtime_config()
        with pytest.raises(NoMatchingContractError):
            build_compose_command(ms, rc, setup_config=None, json_fpath=None)
