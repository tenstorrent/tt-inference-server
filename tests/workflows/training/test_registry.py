...
def test_known_mapping_resolves_to_existing_file():
    path = expected_config_path("Llama-3.1-8B", "p150")
    assert path.is_file()


def test_device_is_case_insensitive():
    lower = expected_config_path("Llama-3.1-8B", "p150")
    upper = expected_config_path("Llama-3.1-8B", "P150")
    assert lower == upper


def test_unknown_mapping_raises():
    with pytest.raises(KeyError):
        expected_config_path("Llama-3.1-8B", "n150")
