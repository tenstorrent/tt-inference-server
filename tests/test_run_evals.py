...
def test_exclude_agentic_eval_tasks_keeps_non_agentic_tasks(monkeypatch):
    run_evals = _import_run_evals(monkeypatch)
    eval_config = EvalConfig(
        hf_model_repo="test/repo",
        tasks=[
            EvalTask(task_name="gpqa"),
            EvalTask(
    ...
    selected_config = run_evals._exclude_agentic_eval_tasks(eval_config)

    assert [task.task_name for task in selected_config.tasks] == ["gpqa"]
    assert selected_config.hf_model_repo == "test/repo"


def test_exclude_agentic_eval_tasks_returns_original_when_none(monkeypatch):
    run_evals = _import_run_evals(monkeypatch)
    eval_config = EvalConfig(
        hf_model_repo="test/repo",
        tasks=[EvalTask(task_name="gpqa")],
    )

    selected_config = run_evals._exclude_agentic_eval_tasks(eval_config)

    assert selected_config is eval_config


def test_build_eval_command_rejects_agentic_tasks(monkeypatch):
    run_evals = _import_run_evals(monkeypatch)
    task = EvalTask(
        task_name="terminal_bench_2",
        workflow_venv_type=WorkflowVenvType.EVALS_AGENTIC,
    ...
    with pytest.raises(ValueError, match="must run via --workflow agentic"):
        run_evals.build_eval_command(
            task=task,
            model_spec=SimpleNamespace(hf_model_repo="test/repo", model_id="test-model"),
            device="gpu",
            output_path="/tmp/evals",
