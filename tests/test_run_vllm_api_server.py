...
def test_apply_local_hf_model_vllm_args_keeps_hf_repo_for_gemma4_plugin_lookup(
    monkeypatch, run_vllm_api_server_module
):
    local_path = "/cache/model_file_symlinks_map/gemma-4-31b-it"
    monkeypatch.setenv("HF_MODEL", local_path)
    model_spec = {"hf_model_repo": "google/gemma-4-31b-it"}
    vllm_args = {
        "model": "google/gemma-4-31b-it",
        "seed": "9472",
    }

    updated = run_vllm_api_server_module.apply_local_hf_model_vllm_args(
        model_spec, vllm_args
    )

    assert updated["model"] == "google/gemma-4-31b-it"
    assert updated["served-model-name"] == "google/gemma-4-31b-it"
    assert updated["seed"] == "9472"
    assert "hf_overrides" not in updated


def test_apply_local_hf_model_vllm_args_applies_metadata_reasoning_parser(
    monkeypatch, run_vllm_api_server_module
):
    monkeypatch.setenv("HF_MODEL", "/cache/model_file_symlinks_map/gemma-4-12b-it")
    model_spec = {
        "hf_model_repo": "google/gemma-4-12b-it",
        "metadata": {"reasoning_parser_name": "gemma4"},
    }
    vllm_args = {"model": "google/gemma-4-12b-it"}

    updated = run_vllm_api_server_module.apply_local_hf_model_vllm_args(
        model_spec, vllm_args
    )

    assert updated["reasoning_parser"] == "gemma4"
    assert "hf_overrides" not in updated


def test_apply_local_hf_model_vllm_args_skips_non_gemma4_models(
    monkeypatch, run_vllm_api_server_module
):
    monkeypatch.setenv("HF_MODEL", "/cache/weights/llama")
    model_spec = {"hf_model_repo": "meta-llama/Llama-3.1-8B-Instruct"}
    ...
    assert updated is vllm_args


def test_apply_local_hf_model_vllm_args_preserves_existing_served_model_name(
    monkeypatch, run_vllm_api_server_module
):
    local_path = "/cache/model_file_symlinks_map/gemma-4-12b-it"
    monkeypatch.setenv("HF_MODEL", local_path)
    model_spec = {"hf_model_repo": "google/gemma-4-12b-it"}
    vllm_args = {
    ...
    assert updated["model"] == "google/gemma-4-12b-it"
    assert updated["served-model-name"] == "custom-name"
    assert "hf_overrides" not in updated


def test_apply_local_hf_model_vllm_args_preserves_existing_hf_overrides(
    monkeypatch, run_vllm_api_server_module
):
    monkeypatch.setenv("HF_MODEL", "/cache/model_file_symlinks_map/gemma-4-12b-it")
    model_spec = {"hf_model_repo": "google/gemma-4-12b-it"}
    existing_overrides = json.dumps({"architectures": ["CustomArch"]})
    vllm_args = {
        "model": "google/gemma-4-12b-it",
    ...
    assert updated["hf_overrides"] == existing_overrides
