...
_METRICS = {
    ...
    "measured_prefix_cache_hit_pct": 98.08,
...
class TestToAgenticSweepPoint:
    ...
    def test_omits_the_cache_hit_rate_when_only_theoretical_is_known(self):
        """The theoretical rate is the reuse offered, not what the cache caught.

        Grading against it would let the target pass on a run where the engine's
        hit rate was never observed, so the field is left out and reported as
        ungraded instead.
        """
        theoretical_only = {
            k: v for k, v in _METRICS.items() if k != "measured_prefix_cache_hit_pct"
        }

        assert "kvCacheHitRatePct" not in to_agentic_sweep_point(
            theoretical_only, concurrency=1
        )
