...
class TestRunLocalServer:
    ...
    def test_build_local_server_env_sets_expected_overrides(self, tmp_path):
        ...
        host_volume = tmp_path / "cache-root"
        model_spec = self._make_model_spec()
        runtime_config = self._make_runtime_config(
            tt_metal_home,
        ...
        assert env["CACHE_ROOT"] == str(host_volume.resolve())
        assert env["TT_METAL_LOGS_PATH"] == str(host_volume.resolve() / "logs")
    ...
    def test_run_local_command_registers_cleanup_for_workflows(self, tmp_path):
        ...
        command = ["/tmp/python", "/tmp/run_vllm_api_server.py", "--model", "repo/model"]
        env = {"PATH": "/tmp"}
        runtime_config = RuntimeConfig(
            model="Mistral-7B-Instruct-v0.3",
            workflow="benchmarks",
        ...
        with patch("workflows.run_local_server.subprocess.Popen", return_value=process), patch(
            "workflows.run_local_server.atexit.register"
