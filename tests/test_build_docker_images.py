...
class TestGemma4RequirementsInstallFlag:
    def test_model_config_requires_gemma4_for_gemma4_repo(self):
        from types import SimpleNamespace

        from scripts.build_docker_images import model_config_requires_gemma4_requirements

        config = SimpleNamespace(
            hf_model_repo="google/gemma-4-31b-it",
            model_name="gemma-4-31b-it",
        )
        assert model_config_requires_gemma4_requirements(config)

    def test_model_config_does_not_require_gemma4_for_llama(self):
        from types import SimpleNamespace
        ...
        assert not model_config_requires_gemma4_requirements(config)

    def test_combination_auto_detect_from_model_configs(self):
        from types import SimpleNamespace

        from scripts.build_docker_images import (
            combination_requires_gemma4_requirements,
            resolve_install_gemma4_requirements,
        )

        configs = {
            "gemma4": SimpleNamespace(
                tt_metal_commit="9970093",
                vllm_commit="3334377",
                hf_model_repo="google/gemma-4-31b-it",
                model_name="gemma-4-31b-it",
            ),
        ...
        assert combination_requires_gemma4_requirements(
            configs, "9970093", "3334377"
        )
        assert not combination_requires_gemma4_requirements(
            configs, "e867533", "8f36910"
        )
        assert not resolve_install_gemma4_requirements(
            configs, "e867533", "8f36910"
        )
        assert resolve_install_gemma4_requirements(
            configs,
            "e867533",
            "8f36910",
            force_install_gemma4_requirements=True,
        )
