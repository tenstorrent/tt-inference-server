...
class TestGetMeshDeviceForHosts:
    def test_dual_galaxy(self):
        assert get_mesh_device_for_hosts(2) == "(8,8)"

    def test_quad_galaxy(self):
        assert get_mesh_device_for_hosts(4) == "(8,16)"

    def test_unsupported(self):
        with pytest.raises(ValueError):
            get_mesh_device_for_hosts(5)
...
class TestAutoGeneratedConfigPklDir:
    ...
    def test_generate_config_pkl_dir_format(self):
        ...
        from workflows.multihost_orchestrator import _generate_config_pkl_dir

        shared_root = "/mnt/shared"
        result = _generate_config_pkl_dir(shared_root)
    ...
    def test_generate_config_pkl_dir_unique(self):
        ...
        shared_root = "/mnt/shared"
        paths = [_generate_config_pkl_dir(shared_root) for _ in range(5)]
    ...
    def test_create_with_permissions_sets_sticky_bit(self):
        ...
        from workflows.multihost_orchestrator import _create_config_pkl_dir_with_permissions

        with tempfile.TemporaryDirectory() as tmpdir:
            config_pkl = Path(tmpdir) / ".tt-inference-server" / "session-test" / "config_pkl"
            ...
            assert session_st.st_mode & stat.S_ISVTX, "Session dir sticky bit should be set"
