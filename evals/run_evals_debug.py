...
def parse_args():
    ...
    # 🔧 DEBUG: Add new debugging options
    parser.add_argument(
        "--limit-samples",
        type=int,
        help="🔧 DEBUG: Limit the number of test samples (e.g., 5 for quick testing)",
        default=None,
    )
    parser.add_argument(
        "--verbose-debug",
        action="store_true",
        help="🔧 DEBUG: Enable verbose debugging to see LLM generations and results",
    )
...
def build_eval_command(
    ...
    # 🔧 DEBUG: Add debugging options to lm_eval command
    if limit_samples:
        cmd.extend(["--limit", str(limit_samples)])
        logger.info(f"🔧 DEBUG: Limiting evaluation to {limit_samples} samples")
    
    if verbose_debug:
        cmd.extend(["--verbosity", "DEBUG"])
        logger.info(f"🔧 DEBUG: Enabling verbose debug output to see LLM generations")
...
def main():
    ...
    logger.info(f"🔧 DEBUG MODE: Running {__file__} with debugging enabled...")

    args = parse_args()
    
    # 🔧 DEBUG: Log debug options
    if args.limit_samples:
        logger.info(f"🔧 DEBUG: Sample limit enabled - will test only {args.limit_samples} problems")
    if args.verbose_debug:
        logger.info(f"🔧 DEBUG: Verbose debugging enabled - will show LLM generations and results")
    ...
    # 🔧 DEBUG: Show what tasks will be evaluated
    logger.info(f"🔧 DEBUG: Found {len(eval_config.tasks)} evaluation tasks for {model_config.model_name}:")
    for task in eval_config.tasks:
        logger.info(f"   - Task: {task.task_name}")
        if hasattr(task, 'score') and task.score:
            logger.info(f"     Expected score: {task.score.published_score}%")
    ...
    logger.info("🔧 DEBUG: Running vLLM evals client with debugging...")
    return_codes = []
    
    for i, task in enumerate(eval_config.tasks, 1):
        health_check = prompt_client.get_health()
        if health_check.status_code != 200:
            logger.error("⛔️ vLLM server is not healthy. Aborting evaluations.")
            return 1

        # 🔧 DEBUG: Enhanced task logging
        logger.info(f"🔧 DEBUG: ===== TASK {i}/{len(eval_config.tasks)}: {task.task_name} =====")
        logger.info(f"🔧 DEBUG: Task details:")
        logger.info(f"   - Name: {task.task_name}")
        logger.info(f"   - Eval class: {task.eval_class}")
        logger.info(f"   - Batch size: {task.batch_size}")
        logger.info(f"   - Few-shot examples: {task.num_fewshot}")
        if hasattr(task, 'score') and task.score:
            logger.info(f"   - Expected published score: {task.score.published_score}%")
            logger.info(f"   - GPU reference score: {task.score.gpu_reference_score}%")
        if args.limit_samples:
            logger.info(f"   - 🔧 DEBUG: Limited to {args.limit_samples} samples (vs full dataset)")
        ...
        logger.info(f"🔧 DEBUG: Executing lm_eval command:")
        logger.info(f"   {' '.join(cmd)}")
        logger.info(f"🔧 DEBUG: This will call the LLM {args.limit_samples or 'ALL'} times to generate code solutions...")
        
        return_code = run_command(command=cmd, logger=logger, env=env_vars)
        
        # 🔧 DEBUG: Log task completion
        if return_code == 0:
            logger.info(f"🔧 DEBUG: ✅ Task {task.task_name} completed successfully")
        else:
            logger.error(f"🔧 DEBUG: ❌ Task {task.task_name} failed with return code {return_code}")
    ...
    if all(return_code == 0 for return_code in return_codes):
        ...
        logger.info(f"🔧 DEBUG: All {len(eval_config.tasks)} tasks completed successfully")
        if args.limit_samples:
            logger.info(f"🔧 DEBUG: Tested {args.limit_samples} samples per task (use full evaluation for production results)")
        main_return_code = 0
    else:
        logger.error(
            f"⛔ evals failed with return codes: {return_codes}. See logs above for details."
        )
        logger.error(f"🔧 DEBUG: Failed tasks: {sum(1 for rc in return_codes if rc != 0)}/{len(return_codes)}")
        main_return_code = 1
