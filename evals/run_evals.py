...
from evals.coco_utils import run_yolov4_coco_evaluation, run_yolov8_coco_evaluation
...
def run_coco_evaluation_task(task: EvalTask, model_spec, cli_args, output_path):
    ...
    if model_spec.model_name in ["yolov8s", "yolov8x", "yolov8s_world"]:
        metrics = run_yolov8_coco_evaluation(
            coco_dataset=coco_dataset,
            service_port=cli_args.get("service_port"),
            output_path=output_path,
            max_images=max_images,
            model_name=f"YOLOv8-{model_spec.model_name}",
        )
    else:
