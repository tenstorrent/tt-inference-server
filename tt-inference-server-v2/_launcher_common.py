...
per-tool venv and ``os.execv`` ``run.py`` inside it, forwarding argv
verbatim. The only difference is which ``WorkflowVenvType`` they pick, so
the sys.path bootstrap, venv setup, and re-exec live here once.
"""
...
def setup_venv_and_exec(venv_type, logger: logging.Logger, label: str) -> int:
    """Materialize ``venv_type`` and re-exec ``run.py`` inside it.

    Imports only the lightweight ``workflows.*`` helpers so the launcher
    runs from any base Python; the venv brings in everything ``run.py`` and
    the perf-tool subprocess actually need. ``VenvConfig.setup`` is
    idempotent, so repeated launches are cheap once the venv exists.
    """
    ...
    logger.info("Launching run.py for %s inside venv: %s", label, venv_python)
    sys.stdout.flush()
    sys.stderr.flush()
    # exec (not subprocess) so the benchmark inherits this process's stdio and
    # exit code directly. Targets run.py, never the launcher, so there is no
