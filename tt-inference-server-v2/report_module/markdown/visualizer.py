...
def build_benchmark_sweeps_markdown(
    ...
    header = f"### Performance Benchmark Sweeps for {model_name} on {device_str}\n\n"
...
def build_benchmark_targets_markdown(
    ...
    if vlm_target_rows:
        ...
        heading = (
            f"#### VLM Performance Benchmark Results {model_name} on {device_str}\n\n"
        )
        target_checks = vlm_target_rows[0].get("target_checks")
        sections.append(
            heading + benchmark_vlm_release_markdown(flat, target_checks=target_checks)
        )
...
def build_percentile_benchmark_markdown(
    ...
    sections: List[str] = [f"### {section_header} for {model_name} on {device_str}\n\n"]
...
def build_parameter_support_markdown(
    ...
    if not reports:
        return ""
    sections = [_build_single_parameter_report(r) for r in reports]
    return "\n---\n\n".join(sections)
...
def _summary_table_rows(summary: Dict[str, Any]) -> List[Dict[str, str]]:
    return [
        ...
        {
            "Metric": "Generated",
            "Value": _fmt_generated(summary.get("generated_at", "")),
        },
    ]
...
def _build_single_parameter_report(report_data: Dict[str, Any]) -> str:
    ...
    summary = _analyze_parameter_report(report_data)
    metadata_md = _format_parameter_metadata(report_data, task_name)
    summary_md = _format_parameter_summary_table(summary, task_name)
    details_md = _format_parameter_detailed_table(summary, task_name)
...
def _analyze_parameter_report(
    ...
    if not isinstance(results, dict):
        raise AttributeError("'results' field missing or invalid in parameter report")
...
def _format_parameter_detailed_table(
    ...
    for test_case in sorted(summary.keys()):
        ...
        for test in sorted_tests:
            ...
            message = (
                _escape_parameter_cell(test["message"]) if status == "FAILED" else ""
            )
            lines.append(
