...
"""Prefix-caching benchmark scenario expansion.
...
driver needs; the driver is intentionally kept dumb about the scenario
...
# ``gamma`` with smoothness <1 produces bursty/clustered traffic (this is
# how AIPerf models "concurrency burst" workloads, per
# ``--arrival-smoothness`` documentation).
...
class PrefixCacheRun:
    ...
    # Trace-driven scenario (mooncake_trace) knobs. When ``trace_input_file``
    # is set the driver switches aiperf to
    # ``--custom-dataset-type mooncake-trace --input-file <trace>`` and
    # ignores the synthetic ISL/OSL flags.
...
def load_manifest(path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    *path* defaults to :data:`DEFAULT_MANIFEST_PATH`, the in-tree manifest
    bundled with the v2 ``llm_module.prefix_cache`` package.
    """
...
def _coerce_lengths(
    value_single: Optional[int],
    values_list: Optional[Sequence[int]],
    fallback: int,
) -> List[int]:
    ...
    for isl_profile in isl_profiles:
        for length in lengths:
            for reuse in reuse_ratios:
                # reuse_ratio = average requests per prefix
                # => pool_size = ceil(N / reuse)
...
def _expand_multi_turn(
    ...
    *,
    ...
    arrivals = cfg.get("arrival_patterns") or [{"pattern": "poisson"}]
    # For multi-turn, total requests ~= conversation_num * turn_mean; we
    # still pass an explicit request_count to AIPerf so it stops at a
    # predictable point.
    request_count = int(
        cfg.get(
...
def _resolve_trace_path(raw: str) -> Path:
    ...
    the package's bundled assets dir (``llm_module/prefix_cache/``), which
    is where in-tree sample traces live.
    """
    p = Path(raw)
    if p.is_absolute():
        return p
...
def _expand_mooncake_trace(
    ...
    *,
    ...
    The manifest entry shape is documented in the v1 docstring; ``traces``
    is a list of ``{"name", "path"}`` (path relative to this package, or
    absolute), ``synthesis_grid`` describes the ``--synthesis-*``
    multiplier variants to sweep, plus ``block_size`` and
    ``fixed_schedule`` knobs. ``trace_path_override`` (from CLI
    ``--prefix-cache-trace``) replaces every manifest-declared trace.
    """
    ...
    # AIPerf 0.5 rejects ``--arrival-pattern`` together with a custom
    # dataset that has timestamps (every mooncake trace does). Trace mode
    # therefore always runs in ``--fixed-schedule`` and the manifest's
    # ``arrival_patterns`` / top-level arrival overrides are ignored for
    # this scenario type.
    fixed_schedule = True
    ...
    for trace in traces_cfg:
        ...
        if not raw_path:
            raise ValueError(
                "mooncake_trace entry is missing 'path' (relative to the "
                "llm_module/prefix_cache/ package, or an absolute path)."
            )
            ...
            for concurrency in concurrencies:
                ...
                runs.append(
                    PrefixCacheRun(
                        ...
                        # the trace's nominal mean (if the manifest
                        # supplies one) so reports stay consistent.
                        ...
                        # ``timestamp`` column when fixed_schedule=True;
                        # we still record "trace" here so reports surface
                        # the provenance.
