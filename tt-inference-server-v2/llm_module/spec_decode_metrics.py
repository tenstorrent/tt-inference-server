...
vLLM does not expose acceptance-rate in its result JSON. Instead, counters
live on the OpenAI-API server's ``/metrics`` Prometheus endpoint. The
spec-decode driver snapshots these counters before and after each ``aiperf
profile`` invocation; the delta gives per-run figures (so long-lived
servers are fine).

Engine-agnostic on purpose: SGLang and other servers that expose
Prometheus text in the same shape can reuse :func:`fetch_prometheus_counters`
directly.
"""
...
def scrape_spec_decode_metrics(
    ...
    Returns a dict with:
        ...
        - ``mean_accepted_length``: ``1 + accepted / num_drafts`` (the ``+1``
          ...
          every draft round — matches vLLM's ``SpecDecodingLogging`` and
          the ``SpecDecodingProm`` doc convention). ``None`` if the server
