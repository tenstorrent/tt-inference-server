...
└── stress_tests_record_builder.py       # Aggregates per-sweep result JSONs into a schema-conformant record for report_module
```
...
- **Record building** (`stress_tests_record_builder.py`): JSON result processing and aggregation into a single schema-conformant record consumed by `report_module/` (which owns rendering)
