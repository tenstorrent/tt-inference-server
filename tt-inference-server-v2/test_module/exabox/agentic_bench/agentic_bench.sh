...
# Concurrency (closed-loop). --profile concurrent holds --rate workers
# in flight at all times; whenever one returns, the next request fires.
# --rate here is an in-flight worker count, not a request rate.
RATE=""                                             # unset => defaults to TARGET_CONCURRENCY
TARGET_CONCURRENCY="${TARGET_CONCURRENCY:-64}"

TURNS="${TURNS:-3}"
PROMPT_TOKENS="${PROMPT_TOKENS:-400}"
PROMPT_TOKENS_STDEV="${PROMPT_TOKENS_STDEV:-150}"
PROMPT_TOKENS_MIN="${PROMPT_TOKENS_MIN:-50}"
PROMPT_TOKENS_MAX="${PROMPT_TOKENS_MAX:-1500}"
OUTPUT_TOKENS="${OUTPUT_TOKENS:-3000}"
OUTPUT_TOKENS_STDEV="${OUTPUT_TOKENS_STDEV:-120}"
OUTPUT_TOKENS_MIN="${OUTPUT_TOKENS_MIN:-1000}"
OUTPUT_TOKENS_MAX="${OUTPUT_TOKENS_MAX:-4096}"
PREFIX_TOKENS="${PREFIX_TOKENS:-300}"
PREFIX_COUNT="${PREFIX_COUNT:-4}"
...
Concurrency (closed-loop; --profile concurrent holds --rate workers in flight):
  --rate N                  In-flight worker count. If not set, defaults to
                            --target-concurrency.
  --target-concurrency N    Default in-flight worker count             [64]
...
resolve() {
  ...
  # --profile concurrent treats --rate as the in-flight worker count.
  # If --rate wasn't given, default it to TARGET_CONCURRENCY.
  if [[ -z "$RATE" ]]; then
    RATE="${TARGET_CONCURRENCY}"
    RATE_DERIVED=1
  else
    RATE_DERIVED=0
  fi
...
save_config() {
  local out="$1/config.txt"
  {
    ...
    echo "# $(date -u +%FT%TZ)"
    for v in TARGET MODEL PROCESSOR RATE TARGET_CONCURRENCY DURATION \
...
main() {
  ...
  if (( RATE_DERIVED == 1 )); then
    echo "==> rate:    ${RATE} in-flight workers (default from --target-concurrency)"
  else
    echo "==> rate:    ${RATE} in-flight workers (explicit --rate)"
  fi
