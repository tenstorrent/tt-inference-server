...
"""Unified benchmark tests for every media type (function-based).

Public entry points:

* ``run_image_benchmark(ctx)``
* ``run_audio_benchmark(ctx)``
* ``run_cnn_benchmark(ctx)``
* ``run_embedding_benchmark(ctx)``
* ``run_tts_benchmark(ctx)``
* ``run_video_benchmark(ctx)``

Each takes a :class:`MediaContext` and performs the same work the legacy
``utils/media_clients/<media>_client.py`` classes did in ``run_benchmark()``.
"""
...
def _common_report_metadata(ctx: MediaContext, task_type: str) -> dict:
    return {
...
def run_image_benchmark(ctx: MediaContext) -> dict:
    ...
    report_data = _common_report_metadata(ctx, "image")
...
def _audio_avg(
    status_list: list[AudioTestStatus], attr: str
) -> float:
    if not status_list:
        return 0.0
    valid = [
        getattr(s, attr) for s in status_list if getattr(s, attr) is not None
    ]
...
def run_audio_benchmark(ctx: MediaContext) -> dict:
    ...
    report_data = _common_report_metadata(ctx, "audio")
    report_data["benchmarks"] = {
        "num_requests": len(status_list),
...
def run_cnn_benchmark(ctx: MediaContext) -> dict:
    ...
    report_data = _common_report_metadata(ctx, "cnn")
    report_data["benchmarks"] = {
        "num_requests": len(status_list),
        "num_inference_steps": status_list[0].num_inference_steps
        if status_list
...
def run_embedding_benchmark(ctx: MediaContext) -> dict:
    ...
    report_data = _common_report_metadata(ctx, "embedding")
    report_data["benchmarks"] = {
        "isl": isl,
...
def run_tts_benchmark(ctx: MediaContext) -> dict:
    ...
    report_data = _common_report_metadata(ctx, "tts")
    report_data["benchmarks"] = {
        "num_requests": len(status_list),
...
def _run_video_generation_benchmark(
    ...
    for i in range(num_calls):
        ...
        status, elapsed, job_id, video_path = _generate_video(
            ctx, prompt=f"Test video generation {i + 1}", num_inference_steps=inference_steps
        )
...
def run_video_benchmark(ctx: MediaContext) -> dict:
    ...
    report_data = _common_report_metadata(ctx, "video")
    report_data["benchmarks"] = {
        "num_requests": len(status_list),
        "num_inference_steps": status_list[0].num_inference_steps
        if status_list
