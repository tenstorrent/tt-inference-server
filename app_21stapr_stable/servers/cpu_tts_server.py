    ...
    try:
        while True:
            ...
            try:
                ...
                # Raw PCM synthesis: skip disk I/O and WAV encoding entirely.
                # Returns: 4-byte big-endian length + raw int16 PCM @ 16kHz
                if request.get("cmd") == "synthesize_raw":
                    import re as _re
                    raw_text = request.get("text", "")
                    raw_sid = request.get("speaker_id")

                    if not raw_text:
                        conn.sendall(b'\x00\x00\x00\x00')
                        continue

                    active_spk = speaker_embeddings
                    if raw_sid is not None and raw_sid in speaker_cache:
                        active_spk = speaker_cache[raw_sid]

                    print(f"[TTS-RAW] {raw_text[:50]}...", flush=True)
                    t0 = time.time()

                    try:
                        max_chars = 400
                        if len(raw_text) > max_chars:
                            sents = _re.split(r'(?<=[.!?])\s+', raw_text)
                            chunks, cur = [], ""
                            for s in sents:
                            ...
                            parts = []
                            for c in chunks:
                                inp = processor(text=c, return_tensors="pt")
                                with torch.no_grad():
                                    parts.append(model.generate_speech(inp["input_ids"], active_spk, vocoder=vocoder).numpy())
                            speech = np.concatenate(parts)
                        else:
                            inp = processor(text=raw_text, return_tensors="pt")
                            with torch.no_grad():
                                speech = model.generate_speech(inp["input_ids"], active_spk, vocoder=vocoder).numpy()

                        pcm_int16 = (np.clip(speech, -1.0, 1.0) * 32767).astype(np.int16)
                        pcm_bytes = pcm_int16.tobytes()
                        elapsed = (time.time() - t0) * 1000
                        print(f"  Raw done {elapsed:.0f}ms ({len(pcm_bytes)} bytes)", flush=True)
