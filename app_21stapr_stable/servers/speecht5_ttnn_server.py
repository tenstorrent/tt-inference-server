...
class SpeechT5TTNNServer:
    ...
    def _normalize_text(text: str) -> str:
        """Normalize text for TTS - convert symbols, numbers, currency to spoken form."""
        import re
        ...
        def _num_to_words(n):
            ...
            if n < 1000:
                return ones[n // 100] + " hundred" + ("" if n % 100 == 0 else " and " + _num_to_words(n % 100))
            if n < 1000000:
                return _num_to_words(n // 1000) + " thousand" + ("" if n % 1000 == 0 else " " + _num_to_words(n % 1000))
            if n < 1000000000:
                return _num_to_words(n // 1000000) + " million" + ("" if n % 1000000 == 0 else " " + _num_to_words(n % 1000000))
            return str(n)
        ...
        def _dollars(m):
            ...
            if "." in amount:
                ...
                result = _num_to_words(dollars) + " dollar" + ("s" if dollars != 1 else "")
                if cents > 0:
                    result += " and " + _num_to_words(cents) + " cent" + ("s" if cents != 1 else "")
                return result
            else:
                n = int(amount)
                return _num_to_words(n) + " dollar" + ("s" if n != 1 else "")

        def _number(m):
            num_str = m.group(0).replace(",", "")
            if "." in num_str:
                parts = num_str.split(".")
                whole = _num_to_words(int(parts[0]))
                decimal = " ".join(_num_to_words(int(d)) if d != "0" else "zero" for d in parts[1])
        ...
        def _percent(m):
            ...
            if "." in num_str:
                parts = num_str.split(".")
                return _num_to_words(int(parts[0])) + " point " + " ".join(_num_to_words(int(d)) if d != "0" else "zero" for d in parts[1]) + " percent"
            return _num_to_words(int(num_str)) + " percent"

        # Currency: $30.97
        text = re.sub(r"\$(\d[\d,]*\.?\d*)", _dollars, text)

        # Percentage: 50%, 3.5%
        text = re.sub(r"(\d[\d,]*\.?\d*)%", _percent, text)

        # Standalone numbers: 10,000 or 3.14 (not inside words)
        text = re.sub(r"(?<!\w)(\d{1,3}(?:,\d{3})+(?:\.\d+)?)(?!\w)", _number, text)
    ...
    def _synthesize(self, text: str, output_path: str):
        ...
        if len(text) * 3 <= self.max_steps:
            ...
            for i, sentence in enumerate(sentences):
                ...
                # Explicitly delete speech tensor to free memory
                del speech
                # Sync device after every 5 segments to prevent trace accumulation
                if (i + 1) % 5 == 0:
        ...
        # Reset KV caches after each synthesis to ensure clean state for next request
        self.generator._reset_kv_caches()
        ...
        try:
            while True:
                ...
                try:
                    ...
                    # Raw PCM synthesis: returns 4-byte big-endian length + int16 PCM @ 16kHz
                    # No disk I/O, no WAV encoding — for WebSocket streaming
                    if req.get("cmd") == "synthesize_raw":
                        import numpy as np
                        raw_text = req.get("text", "")
                        raw_speaker_id = req.get("speaker_id")
                        ...
                        print(f"\n[TTS-RAW #{request_count}] '{raw_text[:50]}{'...' if len(raw_text) > 50 else ''}'")
                        t0 = time.time()

                        try:
                            raw_text = self._normalize_text(raw_text)

                            if raw_speaker_id is not None and raw_speaker_id in self.speaker_cache:
                                speech, _ = self._synthesize_segment_with_speaker(raw_text, raw_speaker_id)
                                audio_np = speech.squeeze().detach().numpy() if hasattr(speech, 'detach') else speech
                            elif len(raw_text) * 3 <= self.max_steps:
                                speech, _ = self._synthesize_segment(raw_text)
                                audio_np = speech.squeeze().detach().numpy()
                                del speech
                            else:
                                ...
                                for s in sentences:
                                    if not s:
                                        continue
                                    sp, _ = self._synthesize_segment(s)
                                    chunks.append(sp.squeeze().detach().numpy())
                                    del sp
                                pause = np.zeros(int(16000 * 0.15), dtype=np.float32)
                                parts = []
                                for i, c in enumerate(chunks):
                                    parts.append(c)
                                    if i < len(chunks) - 1:
                                        parts.append(pause)
                                audio_np = np.concatenate(parts) if parts else np.zeros(1600, dtype=np.float32)
                            ...
                            print(f"  Raw PCM done {elapsed:.0f}ms ({len(pcm_bytes)} bytes)")

                            conn.sendall(len(pcm_bytes).to_bytes(4, 'big'))
                            conn.sendall(pcm_bytes)

                            # Refresh traces AFTER response sent so client isn't blocked
                            if request_count > 1 and request_count % 8 == 0:
                                print(f"  [Post-response] Refreshing traces...")
                                try:
                                    ...
                                    self.generator.capture_all_traces(self.processor, batch_size=1)
                                    self.generator._reset_kv_caches()
                                    print(f"  Traces refreshed!")
                                except Exception as e:
                                    print(f"  Trace refresh failed: {e}")
                        except Exception as e:
                            import traceback
                            failed_requests += 1
                            print(f"  Raw PCM error: {e}")
                            traceback.print_exc()
                            try:
                                self.generator._reset_kv_caches()
                                self.ttnn.synchronize_device(self.device)
