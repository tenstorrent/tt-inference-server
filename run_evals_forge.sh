...
#   cd /home/kmabee/tt-inference-server/tt-media-server
...
#
# Example failing tasks to reproduce for the default model (vs gpu_reference,
# CI run 28352701087): meta_ifeval 74.71 (ratio 0.92), longbench_code_e 14.74
# (0.31), longbench_fewshot_e 31.91 (0.50).
#
...
if [ -n "$TASK" ]; then
  if [ "$SAMPLES" = "all" ]; then
    # "all" = run the full doc set. run.py has no literal "no limit" for a single
    # task, so we send a capped index range that exceeds any single (sub)task's
    # size; lm-eval clamps it to the real count. The cap stays well under the
    # 128KB single-arg execve limit -- a larger range (e.g. 100000) overflows it
    # with "Argument list too long". Tasks bigger than the cap (full mmlu_pro
    # ~12k) won't be fully covered; use --mode or direct lm_eval for those.
    ALL_CAP=4000
    echo "[evals] --samples all -> first $ALL_CAP docs per (sub)task (= full for longbench/gpqa-sized tasks)"
    EVAL_SAMPLES=$(python3 -c "import json,sys; print(json.dumps({'$TASK': list(range(int(sys.argv[1])))}))" "$ALL_CAP")
  else
