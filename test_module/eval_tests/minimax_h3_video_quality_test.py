...
# reference_config keys models by full HF id; MODEL_NAME is the basename the server sees.
ACCURACY_REFERENCE_KEY = "MiniMaxAI/MiniMax-H3"
DURATION_SECONDS = 5
ASPECT_RATIO = "16:9"
...
def _load_quality_reference(requested_count: int) -> dict[str, Any] | None:
    ...
    model_reference = reference_data.get(ACCURACY_REFERENCE_KEY) or reference_data.get(
        MODEL_NAME
    )
