...
# Only a measured hit rate is gradeable. The theoretical rate is what the trace
# pool implies -- the reuse the engine was *offered* -- not what the cache
# caught, so substituting it would let the target pass on a run where the
# engine's hit rate was never observed. Absent a measurement the field is left
# out, which the report renders as an ungraded "N/A ➖" cell.
_CACHE_HIT_METRICS: Tuple[str, ...] = ("measured_prefix_cache_hit_pct",)
