...
**Note:** Custom parameters generate a full cross-product and override automatic parameter generation. Supports partial specification (e.g., only custom-isl-values with auto-generated OSL).
...
Percentiles are calculated for: TTFT, TPOT, ITL, and E2EL. Use `--percentile-report` flag for detailed percentile breakdowns in reports.
...
- **Trace Capture**: One-time upfront trace collection for all unique context lengths, skipped in subsequent subprocess calls
