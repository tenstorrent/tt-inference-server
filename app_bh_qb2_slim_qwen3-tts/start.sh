...
CONTAINER="tt-bh-qb2-qwen3-slim"
IMAGE="ghcr.io/tenstorrent/tt-inference-server/tt-voice-assistant:push-button-v2-slim"
...
# 3b. Verify critical browser voice-detection assets are present (VAD + wake word).
# These are served to the browser; a single missing file (e.g. the ORT jsep .mjs loader)
# silently breaks "voice detection" with a 404. Fail early instead.
echo "       Verifying VAD / wake-word static assets..."
REQUIRED_STATIC=(
    "static/vad/ort.js"
    "static/vad/bundle.min.js"
    "static/vad/vad.worklet.bundle.min.js"
    "static/vad/silero_vad_v5.onnx"
    "static/vad/ort-wasm-simd-threaded.wasm"
    "static/vad/ort-wasm-simd-threaded.mjs"
    "static/vad/ort-wasm-simd-threaded.jsep.wasm"
    "static/vad/ort-wasm-simd-threaded.jsep.mjs"
    "static/wakeword/ort.min.mjs"
    "static/wakeword/melspectrogram.onnx"
    "static/wakeword/embedding_model.onnx"
    "static/wakeword/hey_tt_v3.onnx"
    "static/wakeword/ort-wasm-simd-threaded.wasm"
    "static/wakeword/ort-wasm-simd-threaded.mjs"
    "static/wakeword/ort-wasm-simd-threaded.jsep.wasm"
    "static/wakeword/ort-wasm-simd-threaded.jsep.mjs"
)
MISSING_STATIC=false
for f in "${REQUIRED_STATIC[@]}"; do
    if ! docker exec $CONTAINER test -f "/home/container_app_user/voice-assistant/$f" 2>/dev/null; then
        echo "    MISSING: $f"
        MISSING_STATIC=true
    fi
done
if $MISSING_STATIC; then
    echo "    ERROR: required voice-detection asset(s) missing above."
    echo "    Add them under $LOCAL_APP/<path> and re-run. Aborting."
    exit 1
fi
echo "       All VAD / wake-word assets present."
