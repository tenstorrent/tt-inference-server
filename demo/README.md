...
> **Audience.** This walkthrough is the **developer / local bring-up** path: it runs
> the server from a built tt-metal checkout via `run.py --dev-mode --local-server`.
> Production deployments use the released, version-pinned **Docker image** built by
> `scripts/build_docker_images.py` from the `dots.ocr` prod spec in
> `workflows/model_specs/prod/vlm.yaml` — see the top-level tt-inference-server
> deployment docs for the container run flow. The endpoint API (Steps 3+) is identical
> either way.
