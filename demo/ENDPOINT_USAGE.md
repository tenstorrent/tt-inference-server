...
> **Image resolution.** This guide targets the **resize proxy on `:8001`**, which
> letterboxes any input image to the model's validated (84×132) patch grid before
> forwarding to the server. Sending a native-resolution image directly to the vLLM
> server on `:8000` is **not** supported — off-grid resolutions can overflow L1 and
> crash the vision tower (see `demo/README.md`). Always go through the `:8001` proxy
