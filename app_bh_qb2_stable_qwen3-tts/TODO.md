...
- **JP→EN**: Whisper large-v3 `translate` task (already cached in Docker, 2.9GB)
- **EN→JP**: Need a translation model. Top candidates:
  - `tokyotech-llm/Llama-3.1-Swallow-8B-Instruct-v0.5` — #1 ranked 8B on JP-TL-Bench (8.80/10 EN→JP), Llama 3.1 arch = drop-in compatible
  - `cyberagent/CAT-Translate-1.4b` — tiny 1.4B, MIT license, beats TranslateGemma-12B, bidirectional
  - `voiceping-ai/qwen3-asr-ja-en-speech-translation` — 1.7B, audio-in → translated text out (speech translation in one step)
  - Option C: Use ELYZA as translator (simplest, prompt-based)
