...
yolov6l_tt_server_impl = ImplSpec(
    impl_id="yolov6l_tt_server",
    impl_name="yolov6l-tt-server",
    repo_url="https://github.com/tenstorrent/tt-metal",
    code_path="models/demos/yolov6l",
)
...
tt_server_impl_ids = {"cnn_tt_server", "yolov6l_tt_server", "sdxl_tt_server"}
...
spec_templates = [
    ...
    ModelSpecTemplate(
        weights=["yolov6l"],  # Custom identifier for YOLOv6L model
        tt_metal_commit="v0.62.2",
        impl=yolov6l_tt_server_impl,
        min_disk_gb=5,
        min_ram_gb=2,
        docker_image="sdxl-inf-server_5b4ecc0d3b",
        model_type=ModelTypes.CNN,
        server_type=ServerTypes.TT_SERVER,
        supported_modalities=["image"],
        model_sources=[ModelDownloadSourceTypes.ULTRALYTICS_DOWNLOAD],  # Change from GDRIVE_DOWNLOAD to ULTRALYTICS_DOWNLOAD
        docker_cmd=["/bin/bash", "-c", "source ${PYTHON_ENV_DIR}/bin/activate && cd ${TT_METAL_HOME}/server/ && python3 /home/container_app_user/docker_run_scripts/model_scripts/yolov6l_ci_script.py"],
        device_model_specs=[
            DeviceModelSpec(
                device=DeviceTypes.N150,
                max_concurrency=8,
