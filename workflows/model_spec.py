...
class DeviceModelSpec:
    ...
    # How many server WORKERS must report ready for a "full board" health check.
    # Defaults to max_concurrency, which is correct when each worker owns one
    # chip (DEVICE_IDS='(0),(1),...'): workers == chips == concurrency. It is
    # wrong under SPMD data parallelism, where ONE worker owns the whole device
    # group (DEVICE_IDS='(0,1,2,3)') while max_concurrency still describes
    # request capacity — there, set this to the worker count (1).
