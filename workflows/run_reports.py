...
def benchmark_generate_report(args, server_mode, model_spec, report_id, metadata={}):
    # Look for vLLM and structured-output benchmark files (all stack together)
    # in the same unified output directory. Non-vLLM perf tools (AIPerf,
    # GenAI-Perf, GuideLLM) run through the v2 engine and emit their own
    # reports.
    ...
    # tasks were executed, the report should not surface stale text/vlm JSONs
    # left in the shared benchmarks_output/ directory by prior runs.
    ...
    logger.info(
        f"Found {len(vllm_files)} vLLM, {len(structured_files)} structured-output "
    ...
    logger.info(f"After deduplication: {len(vllm_files)} vLLM benchmark files")
    output_dir = Path(args.output_path) / "benchmarks"

    if not vllm_files and not structured_files:
    ...
    # Process vLLM benchmarks to generate the report tables
    all_tool_results = []
    ...
    # Process all tools and collect results by type (text/image/audio/tts/embedding/cnn/video)
    text_sections = []
    image_sections = []
    audio_sections = []
    tts_sections = []
    ...
    if vllm_files:
        ...
        # Separate text, vlm, asr, tts, embedding, cnn and video for vLLM
    ...
    # Combine sections: text, image, audio, embedding, cnn, then video (matching original order)
    markdown_sections = (
        text_sections
        + image_sections
        + audio_sections
