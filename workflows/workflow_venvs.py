...
def setup_evals_agentic(
    ...
    Harbor is cloned and installed editable (rather than from PyPI) so its
    top-level ``adapters/`` directory is available on disk. The adapters are not
    part of the Harbor wheel and live outside ``src/`` (e.g.
    ``adapters/tau3-bench/tau3_llm_agent.py``), so we also drop a ``.pth`` file
    pointing at the repo root to make ``adapters.<name>...`` importable via
    ``agent_import_path``.
    """
    ...
    # Pin to the Harbor release we validate against; keep in sync with the
    # version the adapters (e.g. tau3-bench) were generated/tested with.
    ...
    # Expose Harbor's top-level adapters/ (outside src/, not in the wheel) so
    # agent_import_path values like "adapters.tau3-bench.tau3_llm_agent:..." resolve.
