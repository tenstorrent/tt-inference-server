...
def setup_evals_ruler(
    ...
    logger.warning("this might take 5 to 15+ minutes to install on first run ...")
    run_command(
        f"{uv_exec} pip install --python {venv_config.venv_python} git+https://github.com/tstescoTT/lm-evaluation-harness.git@adam/ruler#egg=lm-eval[api,ifeval,math,sentencepiece,r1_evals,ruler] protobuf pyjwt==2.7.0 pillow==11.0.0 datasets==3.1.0 wonderwords nltk",
        logger=logger,
    )
    return True
...
_venv_config_list = [
    ...
    VenvConfig(
        venv_type=WorkflowVenvType.EVALS_RULER,
        setup_function=setup_evals_ruler,
    ),
    VenvConfig(
