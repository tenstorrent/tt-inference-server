...
class WorkflowSetup:
    ...
    def run_workflow_script(self, args):
        ...
        if self.workflow_config.workflow_type == WorkflowType.REPORTS:
            ...
            # 🔧 DEBUG: Pass debugging arguments to evals workflow
            if self.workflow_config.workflow_type == WorkflowType.EVALS:
                if hasattr(self.args, "limit_samples") and self.args.limit_samples:
                    cmd += ["--limit-samples", str(self.args.limit_samples)]
                    logger.info(f"🔧 DEBUG: Passing --limit-samples {self.args.limit_samples} to evaluation")
                if hasattr(self.args, "verbose_debug") and self.args.verbose_debug:
                    cmd += ["--verbose-debug"]
                    logger.info(f"🔧 DEBUG: Passing --verbose-debug to evaluation")
