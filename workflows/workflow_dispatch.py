...
def _llm_evals_are_agentic_only(model_spec, runtime_config) -> bool:
    ...
    Some external CI dispatchers expose ``evals`` but not the dedicated
    ``agentic`` workflow.  An agentic-only EvalConfig is unambiguous, so route
    it through the agentic launcher instead of running the standard lm-eval
    workflow and producing no blocks.
    """
    if model_spec.model_type not in _LLM_LIKE_TYPES:
        return False
    cfg = _eval_config_for(model_spec)
...
def build_engine_commands(model_spec, runtime_config, json_fpath) -> list:
    ...
    if wf == WorkflowType.AGENTIC or agentic_eval_compat:
        if agentic_eval_compat:
            logger.info(
                "Routing agentic-only --workflow evals request through the "
                "agentic launcher."
            )
        return [
            VenvCommand(
