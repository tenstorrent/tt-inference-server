...
def _validate_local_vllm_tt_plugin(runtime_config, venv_python: Path):
    ...
    if return_code != 0:
        raise ValueError(
            ...
            "  cd vllm-tt-plugin && git checkout <pinned revision>\n"
            "  source docs/install-vllm-tt.sh\n"
            "Check out the pinned revision rather than the default branch when one "
            "applies: a release or milestone baseline pins the plugin, and the "
            "plugin owns the vLLM version pin, so a different revision is a "
            "different vLLM. Omit the checkout only if nothing pins it for you.\n"
