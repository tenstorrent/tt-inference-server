...
def resolve_compose_vars(
    ...
    if setup_config:
        ...
        if (
            ...
            env["MODEL_WEIGHTS_DIR"] = str(setup_config.container_model_weights_path)
...
def build_compose_command(
    ...
    if setup_config and getattr(setup_config, "host_model_weights_mount_dir", None):
        compose_vars["HOST_MODEL_WEIGHTS"] = str(
            setup_config.host_model_weights_mount_dir
        )
        if getattr(setup_config, "container_model_weights_mount_dir", None):
            compose_vars["CONTAINER_MODEL_WEIGHTS"] = str(
                setup_config.container_model_weights_mount_dir
            )
        ...
        if getattr(setup_config, "container_model_weights_path", None):
            compose_vars["MODEL_WEIGHTS_PATH"] = str(
                setup_config.container_model_weights_path
            )
        if getattr(setup_config, "container_tt_metal_cache_dir", None):
            compose_vars["TT_CACHE_PATH"] = str(
                setup_config.container_tt_metal_cache_dir
            )
    if json_fpath:
        compose_vars["RUNTIME_MODEL_SPEC_JSON"] = str(json_fpath)
        compose_vars["TT_MODEL_SPEC_HOST_PATH"] = str(
            json_fpath
        )  # pre-0.11 contract uses this
...
def run_compose_down(
    compose_files: list, env_file: Optional[Path] = None, timeout: int = 30
):
