...
def main():
    ...
    if runtime_config.docker_server:
        ...
        if runtime_config.print_compose:
            if is_multihost_deployment(runtime_config):
                ...
                run_multihost_validation_subprocess(
                    multihost_config,
                    model_spec=model_spec,
                ...
                for rank, host in enumerate(multihost_config.hosts):
                    print(
                        f"  ssh {host} 'cd <repo> && docker compose -f deploy/docker-compose.multihost-worker.yml up -d'"
                    )
                print(
    ...
    is_compose_run = runtime_config.docker_server and not is_multihost_deployment(
        runtime_config
    )
    main_return_code = 0
    ...
    try:
        ...
        if (
            ...
            and WorkflowType.from_string(runtime_config.workflow)
            not in {
                WorkflowType.SERVER,
