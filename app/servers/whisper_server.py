...
Streaming mode (add "stream": true to request):
  REQ: {"audio_path": "/tmp/audio.wav", "stream": true}
  REP: Multiple newline-delimited JSON responses:
       {"status": "partial", "text": "Hello", "is_final": false}
       {"status": "partial", "text": "Hello world", "is_final": false}
       {"status": "ok", "text": "Hello world!", "is_final": true, "time_ms": 234.5}
"""
...
def main():
    ...
    print("Streaming: {\"audio_path\": \"...\", \"stream\": true}", flush=True)
    ...
    try:
        while True:
            ...
            try:
                ...
                print(f"[Whisper] Transcribing: {audio_path} (stream={stream_mode})", flush=True)
                t0 = time.time()
                ...
                try:
                    ...
                    if stream_mode:
                        # Streaming mode: iterate over generator and send partial results
                        accumulated_text = ""
                        for result in pipeline([(sr, audio)], stream=True):
                            # result format: (transcription_list, avg_logprob, no_speech_prob, is_final)
                            # or with perf metrics: (transcription_list, avg_logprob, no_speech_prob, ttft, throughput, is_final)
                            if len(result) == 4:
                                transcription, avg_logprob, no_speech_prob, is_final = result
                            else:
                                transcription, avg_logprob, no_speech_prob, ttft, throughput, is_final = result
                            
                            # Get text from first batch item
                            partial_text = transcription[0] if transcription else ""
                            ...
                            if is_final:
                                ...
                                print(f"  Final: \"{accumulated_text[:50]}...\" in {elapsed_ms:.1f}ms", flush=True)
                            else:
                                # Partial result
                                response = {
                                    "status": "partial",
                                ...
                                print(f"  Partial: \"{accumulated_text[:30]}...\"", flush=True)
                            
                            # Send newline-delimited JSON for streaming
                            conn.sendall((json.dumps(response) + "\n").encode('utf-8'))
                        
                        # Skip the final sendall since we already sent in the loop
                        continue
                    else:
                        # Non-streaming mode: get full result at once
