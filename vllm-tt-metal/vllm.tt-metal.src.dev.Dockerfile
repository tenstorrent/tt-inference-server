...
RUN /bin/bash -c "git clone https://github.com/tenstorrent/vllm.git ${vllm_dir} \
    ...
    && SETUPTOOLS_SCM_PRETEND_VERSION=0.1.dev0 VLLM_TARGET_DEVICE=empty uv pip install --index-strategy unsafe-best-match -e . --extra-index-url https://download.pytorch.org/whl/cpu \
    && rm -rf ${vllm_dir}/.git"
