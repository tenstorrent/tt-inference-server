...
def _validate_quetzal_auxiliary_references(bundle_manifest: dict,
                                            package_id: str) -> None:
    ...
    if (bundle_manifest.get("external_total_files") != external_files
    ...
    tree_digests = {
        tree.get("role"): tree.get("sha256") for tree in trees
    ...
    auxiliary_identity = hashlib.sha256(_canonical_json([
    ...
    for reference in references:
        ...
        for row in reference["files"]:
            relative = _safe_auxiliary_relative(
                row["path"], f"auxiliary object {name}")
            current = root
            for part in relative.parts[:-1]:
                current = current / part
                if current.is_symlink():
                    raise RuntimeError(
                        f"auxiliary path contains a symlink: {current}")
                try:
                    resolved = current.resolve(strict=True)
                except OSError as exc:
                    raise RuntimeError(
                        f"auxiliary directory is missing: {current}") from exc
            ...
            if not resolved_payload.is_relative_to(resolved_root):
                raise RuntimeError(
                    f"auxiliary object escapes its root: {payload}")
            _require_read_only_path(
                payload, directory=False,
                label=f"auxiliary object {name}/{row['path']}")
