...
class Gemma4ForCausalLM(HybridAttentionForCausalLM):
    ...
    # Gemma4 overrides ``get_kv_cache_spec`` with real SlidingWindowSpec +
    # FullAttentionSpec entries. The base class leaves this False and emits
    # uniform FullAttentionSpec, which breaks warmup broadcast and leaves
    # sliding layers on the legacy single-page-table path while vLLM allocates
    # hybrid pools — decode trace then binds the wrong KV routing path.
    ...
    def initialize_vllm_model(
        ...
        # Keep vLLM ``--model`` as the HF repo id for TT plugin registration;
        # load cached safetensors from ``HF_MODEL`` when ``model_setup()`` set it.
        model_path = os.getenv("HF_MODEL") or hf_config._name_or_path
    ...
    def warmup_model_decode(self, *args, **kwargs):
        ...
        Without this, inherited warmup captures the decode trace while
        ``_persistent_per_layer_page_tables`` do not exist yet (lazy alloc on
        first request). The trace binds the legacy single-page_table path; the
        per-layer buffers are allocated after capture, so hybrid full-attention
        layers read wrong KV blocks at decode time.

        Mirror ``test_vllm_parity.py``: build per-layer tables from the legacy
        warmup ``page_table`` (broadcast + kv-share alias), allocate persistent
        device tensors, then stash on the model before trace capture runs.
        """
        num_blocks = kwargs.get("num_blocks")
        max_batch_size = kwargs.get("max_batch_size", 1)
        if num_blocks is None and len(args) >= 4:
            num_blocks = args[3]
        ...
        if num_blocks is not None:
            ...
            if per_submesh is not None:
                for m, pt_for_submesh in zip(self.model, per_submesh):
                    m._page_tables_to_ttnn(pt_for_submesh)
                    m.update_persistent_per_layer_page_tables(pt_for_submesh)
                    m._active_page_tables_per_layer = pt_for_submesh
        try:
