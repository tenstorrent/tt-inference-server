...
class TTModelRunner:
    ...
    def _validate_kv_cache_groups(kv_cache_groups: list) -> None:
        ...
        for group in kv_cache_groups:
            # UniformTypeKVCacheSpecs wraps several same-type (all-Full) specs
            # that differ only in shape (e.g. Gemma4 with hybrid groups
            # disabled: sliding layers 8x256, full layers 1x512). vLLM merges
            # them into one group; we unwrap to per-layer specs in
            # ``_build_per_layer_specs``, so accept it here.
            if not isinstance(
    ...
    def _build_per_layer_specs(
        ...
        if len(kv_cache_groups) == 1:
            ...
            # wrapped per-layer specs have heterogeneous shapes (Gemma4
            # hybrid-off: sliding 8x256 vs full 1x512). Every layer indexes the
            # same ``num_blocks`` block table but gets its own DRAM buffer sized
            # to its own spec, so ``tensor_idx == layer_idx`` (matching the
            # uniform single-group convention above).
