...
setup_weights_huggingface() {
    echo "⛔ This path is deprecated. Hugging Face downloads are now handled by the Python flow using uv-managed venvs."
    echo "   Please use: python run.py --model <MODEL> --workflow server --docker-server"
    echo "   Or invoke the host setup via workflows/setup_host.py within the Python flow."
    exit 1
}
