...
MAX_HISTORY_TURNS = 4  # Keep last 4 turns (8 messages) to fit in 512 token context
...
async def chat(request: Request):
    """Generate chat response using Llama with conversation context."""
    try:
        data = await request.json()
        user_message = data.get("message", "")
        max_tokens = data.get("max_tokens", 150)
        session_id = data.get("session_id", "default")  # Use session_id for multi-user support
        ...
        # Add user message to history (truncate long messages to save context)
        ...
        result = await llama_service.generate_response(
            ...
            conversation_history=history[:-1]  # Pass history excluding current message
        )
        
        assistant_response = result.get("response", "")
        
        # Add assistant response to history (truncate to save context for next turn)
