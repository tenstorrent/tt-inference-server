...
class LlamaService:
    ...
    def format_prompt(self, user_message: str, conversation_history: list = None) -> str:
        """Format prompt for Llama 3.1 Instruct with conversation history."""
        prompt = """<|begin_of_text|><|start_header_id|>system<|end_header_id|>

You are a helpful voice assistant. Give brief, direct answers suitable for speech. Remember the context of our conversation.<|eot_id|>"""
...
"""
    ...
    async def generate_response(self, message: str, max_tokens: int = 256, conversation_history: list = None) -> Dict[str, Any]:
        """Generate response using Llama with optional conversation context."""
        ...
        try:
            ...
            formatted = self.format_prompt(message, conversation_history)
            tokens = self.tokenizer.encode(formatted)
            
            # Check if we're within context limits (512 tokens max)
            max_input_tokens = 512 - max_tokens - 10  # Leave room for generation + buffer
            if len(tokens) > max_input_tokens:
                logger.warning(f"Input tokens ({len(tokens)}) exceeds limit ({max_input_tokens}), truncating history")
                # Re-format with no history to fit
                formatted = self.format_prompt(message, None)
                tokens = self.tokenizer.encode(formatted)
