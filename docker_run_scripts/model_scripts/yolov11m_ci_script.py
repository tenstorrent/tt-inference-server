...
def create_dummy_eval_output(model_spec, output_path):
    """Create dummy evaluation output files for YOLOv11m."""
    logger.info("Creating dummy evaluation output for YOLOv11m...")
    ...
    # Create dummy COCO detection results for YOLOv11m (better accuracy than YOLOv11n)
    eval_results = {
        ...
        "task_results": {
            "coco_detection_val2017": {
                "mAP": 0.501,  # YOLOv11m expected mAP (higher than YOLOv11n)
                "mAP_50": 0.708,
                "mAP_75": 0.542,
                "mAP_small": 0.315,
                "mAP_medium": 0.552,
                ...
                "inference_time_avg": 0.065,  # seconds per image (slower than YOLOv11n)
                "preprocessing_time_avg": 0.008,
                "postprocessing_time_avg": 0.015
            }
        },
        "summary": {
            "total_tasks": 1,
...
def run_benchmarks_workflow(model_spec, workflow_logs_dir):
    ...
    logger.info("Running benchmarks workflow for YOLOv11m...")
    ...
    # If no configurations found in reference, use defaults for YOLOv11m
    if not configurations:
        logger.warning(f"No benchmark configurations found for {model_spec.model_name} on {device_name}. Using YOLOv11m defaults.")
        configurations = [
            CNNBenchmarkConfig(
    ...
    # Create dummy benchmark results for YOLOv11m
    benchmark_results = []
    for i, config in enumerate(configurations):
        # Simulate YOLOv11m performance (slower but more accurate than YOLOv11n)
        base_latency = 0.065  # 65ms base latency for 640x640 (slower than YOLOv11n)
        latency_with_concurrency = base_latency * (1 + (config.concurrent_requests - 1) * 0.12)
        
        result = CNNBenchmarkResult(
            config=config,
            latencies=[latency_with_concurrency + random.uniform(-0.008, 0.008) for _ in range(config.num_iterations)],
...
def run_evals_workflow(model_spec, workflow_logs_dir):
    """Run evaluations workflow for YOLOv11m."""
    logger.info("Running evaluations workflow for YOLOv11m...")
...
def run_release_workflow(model_spec, workflow_logs_dir):
    """Run release workflow (benchmarks + evals + report generation) for YOLOv11m."""
    logger.info("Running release workflow for YOLOv11m...")
    ...
    # Create YOLOv11m release report
    release_report = {
        "report_id": report_id,
        "model_name": model_spec.model_name,
        "model_id": model_spec.model_id,
        ...
        "timestamp": timestamp,
        "benchmarks_summary": {
            "avg_latency": 0.065,  # YOLOv11m latency (slower than YOLOv11n)
            "throughput": 15.4,    # req/s (lower than YOLOv11n)
            "success_rate": 1.0
        },
        "evals_summary": {
            "coco_detection_val2017": {
                "mAP": 0.501,  # Higher accuracy than YOLOv11n
                "mAP_50": 0.708,
                "mAP_75": 0.542
            }
        },
...
def main():
    """Main function to run the YOLOv11m CI script."""
    logger.info("Starting YOLOv11m CI script...")
