...
Every image, video, and audio file in this directory (staged (not in git) or
regenerated by `make_assets.sh`) is derived from one of two CC-BY / public-domain sources.
...
| File | Staged or generated | Source | Derivation |
|---|---|---|---|
| `img_std_city_skyline.jpg` | staged (not in git) | Tears of Steel | frame @ t=70s, centered-crop -> 1344x768 |
| `img_std_old_man_portrait.jpg` | staged (not in git) | Tears of Steel | frame @ t=175s, centered-crop -> 1344x768 |
| `img_std_three_people.jpg` | staged (not in git) | Tears of Steel | frame @ t=240s, centered-crop -> 1344x768 |
| `img_std_robot_street.jpg` | staged (not in git) | Tears of Steel | frame @ t=300s, centered-crop -> 1344x768 |
| `img_std_young_man_canal.jpg` | staged (not in git) | Tears of Steel | frame @ t=520s, centered-crop -> 1344x768 |
| `img_min_256px_scientist.jpg` | staged (not in git) | Tears of Steel | frame @ t=400s, centered-crop, downscaled -> 456x256 |
| `img_std_earth_from_iss.jpg` | staged (not in git) | NASA `iss007e07306` | centered-crop -> 1344x768 |
| `img_std_hurricane_from_iss.jpg` | staged (not in git) | NASA `iss009e24868` | centered-crop -> 1344x768 |
...
| `vid_min_2s_city_skyline.mp4` | staged (not in git) | Tears of Steel | 2.0s clip, centered-crop+scale -> 1280x720, H.264+AAC (same "city" scene as the still) |
| `vid_std_5s_man_talking.mp4` | staged (not in git) | Tears of Steel | 5.0s clip, centered-crop+scale -> 1280x720, H.264+AAC |
...
| `aud_min_2s_score.wav` | staged (not in git) | Tears of Steel soundtrack | 2s clip @ t=100s |
| `aud_std_5s_score.wav` | staged (not in git) | Tears of Steel soundtrack | 5s clip @ t=360s |
| `aud_max7_score.wav` | staged (not in git) | Tears of Steel soundtrack | 7.04s clip, 32 kHz stereo; required by `REF2VA-H`/`REF2VA-H5`/`REF2VA-H10` (with `aud_min` + `aud_std` = 14.04 s, under the 15 s audio-total cap) and pinned in both manifests. NOT a prefix of `aud_max_8s_score.wav`; offset not recorded when the pack was assembled |
| `aud_max_8s_score.wav` | staged (not in git) | Tears of Steel soundtrack | 8s clip @ t=700s; kept for reference, no case uses it |
...
(`-c:v copy -an`, no re-encoding) of already-staged files and ARE
...
`test_module/_test_common/minimax_h3_bench/cases.json` for why silent reference videos exist at
