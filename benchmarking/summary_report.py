...
def extract_params_from_filename(filename: str) -> Dict[str, Any]:
    ...
    aiperf_image_pattern = r"""
        ...
        (?:_(?P<device>N150|N300|P100|P150|T3K|p150x4|p150x8|p300x2|P300x2|p300|P300|n150x4|TG|GALAXY|n150|n300|p100|p150|t3k|tg|galaxy|gpu|GPU|super_cluster|SUPER_CLUSTER|Super-Cluster))?  # Optional device
    ...
    guidellm_image_pattern = r"""
        ^guidellm_benchmark_
        (?P<model>.+?)                            # Model name (non-greedy, allows everything)
        (?:_(?P<device>N150|N300|P100|P150|T3K|p150x4|p150x8|p300x2|P300x2|p300|P300|n150x4|TG|GALAXY|n150|n300|p100|p150|t3k|tg|galaxy|super_cluster|SUPER_CLUSTER|Super-Cluster))?  # Optional device
        _(?P<timestamp>\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2})
        _isl-(?P<isl>\d+)
        _osl-(?P<osl>\d+)
        _maxcon-(?P<maxcon>\d+)
