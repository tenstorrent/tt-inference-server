...
Every release gets its own dashboard, titled and keyed by `<namespace>/<release>`, so two releases of one engine sit side by side instead of overwriting each other. Each one opens on the release that shipped it: its `job` variable — Prometheus names the job after the `PodMonitor`, which is the release — is preselected. Widen it to `All`, or pick another job, to compare releases in one view.
